...
import { MeanVarianceOptimizer, RiskParityOptimizer, BlackLittermanOptimizer, MultiObjectiveOptimizer, } from '../src/optimizer.js';
describe('Portfolio Optimizers', () => {
    ...
    beforeEach(() => {
        ...
        historicalReturns = Array(252).fill(0).map(() => assets.map(() => (Math.random() - 0.5) * 0.02));
    });
    ...
    describe('BlackLittermanOptimizer', () => {
        it('should incorporate investor views', () => {
            const optimizer = new BlackLittermanOptimizer(assets, correlationMatrix, marketCapWeights, 2.5);
            const views = [
        ...
        it('should work without views (equilibrium)', () => {
            const optimizer = new BlackLittermanOptimizer(assets, correlationMatrix, marketCapWeights);
            const result = optimizer.optimize([]);
            expect(result).toBeDefined();
            expect(result.weights.reduce((a, b) => a + b, 0)).toBeCloseTo(1.0, 2);
        });
        it('should adjust based on risk aversion', () => {
            const conservativeOptimizer = new BlackLittermanOptimizer(assets, correlationMatrix, marketCapWeights, 5.0);
            const aggressiveOptimizer = new BlackLittermanOptimizer(assets, correlationMatrix, marketCapWeights, 1.0);
    ...
    describe('MultiObjectiveOptimizer', () => {
        it('should optimize multiple objectives', () => {
            const optimizer = new MultiObjectiveOptimizer(assets, correlationMatrix, historicalReturns);
            const result = optimizer.optimize({
                return: 1.0,
                risk: 1.0,
                drawdown: 0.5,
