...
export class SelfLearningOptimizer {
    ...
    learningPlugin;
    namespace;
    constructor(dbPath = './portfolio-memory.db', namespace = 'portfolio-optimization') {
     ...
     */
    async learn(result, actualPerformance, marketConditions) {
        const state = await this.loadLearningState();
        if (!state)
            throw new Error('Learning state not initialized');
        ...
        this.learningPlugin.updateModel(stateVector, action, reward, this.encodeState(state, marketConditions), // next state
        false);
     ...
     */
    encodeState(state, marketConditions) {
    ...
    calculateReward(performance) {
        return (performance.sharpeRatio * 1.0 +
     ...
     */
    adaptRiskProfile(state, action, performance) {
     ...
     */
    selectBestAlgorithm(successRates) {
        return Object.entries(successRates).reduce((best, [algo, rate]) => rate > successRates[best] ? algo : best, 'mean-variance');
    }
    /**
     ...
     */
    async retrieveSimilarExperiences(currentState, limit = 10) {
     ...
     */
    async getRecommendedProfile(marketConditions) {
        const state = await this.loadLearningState();
        if (!state)
            throw new Error('Learning state not initialized');
...
export class AdaptiveRiskManager {
    learningOptimizer;
    constructor(learningOptimizer) {
        this.learningOptimizer = learningOptimizer;
    }
    /**
     * Calculate adaptive position sizes
     */
    async calculatePositionSizes(baseWeights, marketConditions, confidenceLevel = 0.95) {
     ...
     */
    calculateRiskMultiplier(profile, marketConditions) {
     ...
     */
    async shouldRebalance(currentWeights, targetWeights, threshold = 0.05) {
        const maxDeviation = currentWeights.reduce((max, w, i) => Math.max(max, Math.abs(w - targetWeights[i])), 0);
