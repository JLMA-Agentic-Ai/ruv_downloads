...
import { MeanVarianceOptimizer, RiskParityOptimizer, BlackLittermanOptimizer, MultiObjectiveOptimizer, } from './optimizer.js';
/**
...
export class PortfolioOptimizationSwarm {
    openai = null;
    learningOptimizer;
    constructor(openRouterApiKey, learningOptimizer) {
        if (openRouterApiKey) {
    ...
    async runBenchmark(config) {
        ...
        for (const algorithm of config.algorithms) {
            for (const constraints of config.constraintVariations) {
                tasks.push(this.runOptimization(algorithm, config, constraints));
            }
        }
     ...
     */
    async runOptimization(algorithm, config, constraints) {
        const startTime = Date.now();
        let result;
        switch (algorithm) {
            case 'mean-variance': {
            ...
            case 'black-litterman': {
                ...
                const optimizer = new BlackLittermanOptimizer(config.assets, config.correlationMatrix, marketCapWeights);
                result = optimizer.optimize([], constraints);
                break;
            }
            case 'multi-objective': {
                const historicalReturns = config.historicalReturns || this.generateMockReturns(config.assets.length);
                const optimizer = new MultiObjectiveOptimizer(config.assets, config.correlationMatrix, historicalReturns);
     ...
     */
    generateMockReturns(numAssets, numPeriods = 252) {
        const returns = [];
        for (let t = 0; t < numPeriods; t++) {
            const periodReturns = Array(numAssets).fill(0).map(() => (Math.random() - 0.5) * 0.02 // -1% to +1% daily returns
            );
    ...
    analyzeResults(results, config) {
        ...
        const bestResult = results.reduce((best, current) => current.result.sharpeRatio > best.result.sharpeRatio ? current : best);
     ...
     */
    async getAIRecommendations(insights, config) {
        if (!this.openai)
            return [];
        const prompt = `
...
`;
     ...
     */
    async learnFromBenchmark(results) {
        if (!this.learningOptimizer)
            return;
     ...
     */
    async exploreConstraints(baseConfig, parameterRanges, samples = 20) {
     ...
     */
    async compareMarketRegimes(config, regimes) {
...
export class ParallelPortfolioExplorer {
     ...
     */
    async optimizeInParallel(configs) {
        const results = [];
        ...
        for (let i = 0; i < configs.length; i += this.maxWorkers) {
            ...
            const batchResults = await Promise.all(batch.map(({ algorithm, config, constraints }) => swarm['runOptimization'](algorithm, config, constraints)));
            results.push(...batchResults);
        }
        return results;
    }
}
//# sourceMappingURL=benchmark-swarm.js.map