 ...
 */
import { PortfolioOptimizationSwarm, } from '../src/benchmark-swarm.js';
...
async function main() {
    ...
    const learningOptimizer = new SelfLearningOptimizer('./swarm-portfolio-memory.db', 'swarm-portfolio');
    ...
    console.log('-'.repeat(70));
    const constraintInsights = await swarm.exploreConstraints(benchmarkConfig, {
        minWeight: [0.02, 0.15],
        maxWeight: [0.30, 0.60],
    ...
    const newProfile = await learningOptimizer.learn(bestResult, mockPerformance, marketConditions);
...
//# sourceMappingURL=swarm-exploration.js.map