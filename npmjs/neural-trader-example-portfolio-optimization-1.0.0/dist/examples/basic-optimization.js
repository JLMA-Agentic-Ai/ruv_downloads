...
async function main() {
    ...
    assets.forEach(asset => {
        console.log(`  ${asset.symbol}: Expected Return ${(asset.expectedReturn * 100).toFixed(2)}%, ` +
    ...
    const blOptimizer = new BlackLittermanOptimizer(assets, correlationMatrix, marketCapWeights, 2.5);
    ...
    const moOptimizer = new MultiObjectiveOptimizer(assets, correlationMatrix, historicalReturns);
 ...
 * Generate mock historical returns
 */
function generateHistoricalReturns(numAssets, numPeriods) {
    const returns = [];
