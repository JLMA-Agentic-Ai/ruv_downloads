...
exports.SwarmFeatureEngineer = void 0;
const child_process_1 = require("child_process");
class SwarmFeatureEngineer {
    agents = [];
    featureSets = [];
    config;
    generation = 0;
    ...
    async initialize() {
        ...
        try {
            (0, child_process_1.execSync)('npx claude-flow@alpha hooks pre-task --description "Market microstructure feature engineering"', {
                encoding: 'utf-8'
            });
        }
        catch (error) {
     ...
     */
    async exploreFeatures(metricsHistory) {
     ...
     */
    async optimizeFeatures(baseFeatures, targetMetric) {
     ...
     */
    async evaluateAgents(metricsHistory) {
     ...
     */
    evaluateAgent(agent, metricsHistory) {
     ...
     * Report progress via claude-flow
     */
    async reportProgress(generation) {
        const bestAgent = [...this.agents].sort((a, b) => b.performance - a.performance)[0];
        try {
            (0, child_process_1.execSync)(`npx claude-flow@alpha hooks notify --message "Generation ${generation}: Best performance ${bestAgent.performance.toFixed(3)}, Features: ${bestAgent.features.length}"`, { encoding: 'utf-8' });
        }
        catch (error) {
     ...
     */
    async enhanceWithOpenRouter(metricsHistory) {
     ...
     */
    async enhanceAnomalyExplanation(metrics, anomalyType) {
    ...
    async finalizeFeatureSets() {
        ...
        try {
            ...
            (0, child_process_1.execSync)(`npx claude-flow@alpha hooks post-edit --file "swarm-features.json" --memory-key "market-microstructure/feature-sets"`, { encoding: 'utf-8' });
        }
        catch (error) {
     ...
     */
    randomFeatureSubset(features, minSize, maxSize) {
     ...
     */
    createOptimizationAgents(baseFeatures) {
        const agents = [];
     ...
     */
    async evaluateOptimizationAgent(agent, targetMetric) {
     ...
     */
    applyAgentFeatures(features, agent) {
        // Modify features based on agent's feature selection
        const modifier = agent.performance * 0.1;
        agent.features.forEach(featureName => {
            if (featureName in features && typeof features[featureName] === 'number') {
                features[featureName] *= (1 + modifier);
            }
        });
    }
    /**
     * Evolve optimization agents
     */
    evolveOptimizationAgents(agents) {
     ...
     */
    detectAnomalyWithAgent(agent, metrics) {
    ...
    async cleanup() {
        try {
            (0, child_process_1.execSync)('npx claude-flow@alpha hooks post-task --task-id "market-microstructure-features"', {
                encoding: 'utf-8'
            });
        }
        catch (error) {
...
exports.SwarmFeatureEngineer = SwarmFeatureEngineer;
//# sourceMappingURL=swarm-features.js.map