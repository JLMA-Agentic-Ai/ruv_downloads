...
export class HybridAgentDBMemory {
    ...
    async storeEpisode(episode) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(episode.sessionId, episode.taskName, episode.confidence, episode.success ? 1 : 0, episode.outcome, episode.strategy || null, episode.metadata ? JSON.stringify(episode.metadata) : null);
     ...
     */
    async bulkStoreEpisodes(episodes) {
        const startTime = performance.now();
        const stmt = this.db.prepare(`
      INSERT INTO reflexion_episodes
      (session_id, task_name, confidence, success, outcome, strategy, metadata)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        const insertMany = this.db.transaction((episodes) => {
     ...
     */
    async retrieveMemories(query, k = 5, options = {}) {
        const startTime = performance.now();
...
//# sourceMappingURL=hybrid-memory.js.map