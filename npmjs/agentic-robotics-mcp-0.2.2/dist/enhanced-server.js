...
export class EnhancedROS3McpServer {
    ...
    config;
    constructor(config = {}) {
        const { name = 'enhanced-ros3-mcp-server', version = '2.0.0', dbPath = './ros3-agentdb.db', enableFlow = true, numAgents = 66, enableReasoning = true, enableLearning = true, } = config;
        this.name = name;
    ...
    async moveRobot(params) {
        ...
        let pastExperiences = '';
        if (useMemory) {
            const similar = await this.memory.retrieveMemories(`move to [${x}, ${y}, ${z}]`, 3, { onlySuccesses: true, synthesizeContext: true });
            if (similar.length > 0) {
    ...
    async readLidar(params) {
        ...
        const startTime = Date.now();
        const cloud = await this.ros3.getLidarData(filter, max_points);
        const latency = Date.now() - startTime;
        if (useMemory) {
            await this.memory.storeEpisode({
    ...
    async detectObjects(params) {
        ...
        const startTime = Date.now();
        const detections = await this.ros3.detectObjects(camera, confidence_threshold);
        const latency = Date.now() - startTime;
        if (useMemory) {
            await this.memory.storeEpisode({
...
${result.reasoning ? `\nReasoning: ${result.reasoning}` : ''}`;
    ...
    async executeSwarm(params) {
        ...
        const tasks = params.tasks.map((t, i) => ({
            id: `swarm-${i}`,
            type: t.type,
            priority: t.priority,
            params: t.params,
...
Total: ${storeTime + retrieveTime}ms`;
    ...
    getInfo() {
        ...
        if (this.config.enableFlow) {
            tools.push('execute_task', 'execute_swarm', 'coordinate_robots', 'reason_about_task', 'list_mcp_tools');
        }
        return {
            name: this.name,
