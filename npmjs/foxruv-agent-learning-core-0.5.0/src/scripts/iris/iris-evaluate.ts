...
import { createIrisPrime, type IrisPrimeConfig } from '../../orchestrators/iris-prime'
import { logTelemetry } from '../../supabase'
import { initSupabaseFromEnv, isSupabaseInitialized } from '../../supabase/client'
  ...
  try {
    ...
    if (!isSupabaseInitialized()) {
      try {
        ...
        console.warn('⚠️  Supabase credentials not configured, using local-only mode')
    ...
    log('IRIS Prime initialized', logPath, options.verbose)
    ...
    // Log results
    log(`Evaluation complete`, logPath, true)
    ...
    log(`Prompt Recommendations: ${report.promptRecommendations.length}`, logPath, true)
    log(`Rotation Recommendations: ${report.rotationRecommendations.length}`, logPath, true)
