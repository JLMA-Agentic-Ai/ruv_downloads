# IRIS Prime CLI Scripts

Three command-line tools for invoking Iris Prime evaluation from hooks, scheduled tasks, or manual operations.

## Scripts Overview

### 1. `iris-evaluate.ts` - Single Project Evaluation

Evaluates a single project's health using Iris Prime.
...
Uses intelligent trigger engine to decide if Iris Prime should be invoked based on event type, context, and recent history. Optimized for fast decision-making (<100ms) to be used in hooks.
