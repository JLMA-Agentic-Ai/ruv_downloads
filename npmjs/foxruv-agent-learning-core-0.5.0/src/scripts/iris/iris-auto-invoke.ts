 ...
 *
 * Uses intelligent trigger engine to decide if Iris Prime should be invoked
...
async function main() {
  ...
  try {
    ...
    if (options.dryRun) {
      log('Dry run: Would invoke Iris Prime', logPath, true)
      process.exit(0)
    }

    // Invoke Iris Prime
    log('Invoking Iris Prime evaluation...', logPath, true)
