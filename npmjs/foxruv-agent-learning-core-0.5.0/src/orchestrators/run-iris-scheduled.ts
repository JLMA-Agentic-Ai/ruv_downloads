/**
 * Scheduled IRIS Prime Runner
 *
 * Runs IRIS Prime on a schedule (daily, hourly, etc.) to continuously monitor
...
import { createIrisPrime, type IrisPrime, type IrisReport, type CrossProjectReport } from './iris-prime'
import { buildDailyDigest, formatDailyDigest } from '../notifications/digest-builder'
import { createWhatsAppNotifier } from '../notifications/whatsapp-notifier'
import { initSupabaseFromEnv } from '../supabase/client'
...
class ScheduledIrisRunner {
  ...
  constructor(config: Partial<ScheduleConfig> = {}) {
    ...
    console.log('🤖 IRIS Prime Scheduled Runner initialized')
  ...
  async runOnce(): Promise<void> {
    ...
    console.log('\n' + '='.repeat(80))
    console.log(`IRIS PRIME - Scheduled Run`)
    ...
    try {
      ...
      console.warn('⚠️  Supabase initialization failed, will use local AgentDB only:', error)
    }
  ...
  stop(): void {
    if (this.intervalId) {
      ...
      console.log('🛑 IRIS Prime scheduler stopped')
    }
  }
