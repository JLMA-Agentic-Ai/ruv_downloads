...
import { getSupabase, getProjectId, getTenantId } from './client';
import { ModelRunLog } from './types';
 ...
 * @returns The created telemetry log entry
 ...
 * ```
 */
export async function logTelemetry(data: TelemetryData): Promise<ModelRunLog> {
  ...
  return await withRetry(async () => {
    ...
    if (error) throw new Error(`Failed to log telemetry: ${error.message}`);
    return result;
...
export async function getExpertStats(
  ...
  const avgConfidence =
    data.reduce((sum, r) => sum + (r.confidence || 0), 0) / totalRuns;
  const avgLatencyMs =
    data.reduce((sum, r) => sum + (r.latency_ms || 0), 0) / totalRuns;
...
export async function detectDrift(
  ...
  const baselineConfidence =
    baselineData.reduce((sum, r) => sum + r.confidence, 0) / baselineData.length;
  const recentConfidence =
    recentData.reduce((sum, r) => sum + r.confidence, 0) / recentData.length;
