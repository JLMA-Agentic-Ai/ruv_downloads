...
export class GlobalMetricsCollector {
  ...
  private async updateExpertMetrics(
    ...
    const stats = stmt.get(project, expertId, version) as any
  ...
  private async checkForDrift(
    ...
    const baseline = baselineStmt.get(project, expertId, version, cutoff, cutoff + halfWindow) as any
    const recent = recentStmt.get(project, expertId, version, cutoff + halfWindow) as any
  ...
  async getCrossProjectMetrics(expertType: string): Promise<CrossProjectMetrics> {
    ...
    const stats = stmt.get(`%${expertType}%`) as any
    ...
    const best = bestStmt.get(`%${expertType}%`) as any
