 ...
 * @version 2.0.0
 */
import { logTelemetry, getExpertStats, detectDrift } from '../supabase/telemetry';
import { getSupabase, isSupabaseInitialized } from '../supabase/client';
import { AgentDBSingleton } from '../storage/agentdb-singleton';
/**
...
export class GlobalMetricsCollector {
    ...
    constructor(config = {}) {
        ...
        if (this.useSupabase) {
            ...
            console.warn('⚠ GlobalMetrics: Supabase not available, using AgentDB only');
        }
    }
    /**
    ...
    async updateExpertMetrics(project, expertId, version) {
      ...
      LIMIT 1000
    `);
        const stats = stmt.get(project, expertId, version);
    ...
    `);
        ...
        const baseline = baselineStmt.get(project, expertId, version, cutoff, cutoff + halfWindow);
        const recent = recentStmt.get(project, expertId, version, cutoff + halfWindow);
    ...
    `);
        const stats = stmt.get(`%${expertType}%`);
      ...
      LIMIT 1
    `);
        const best = bestStmt.get(`%${expertType}%`);
        // Get recent alerts
