#!/usr/bin/env node
/**
 * MCP Wrapper Generator for IRIS Prime
 *
 * Generates TypeScript wrappers for MCP servers configured in:
 * - .mcp.json (project-level)
 * - ~/.claude.json (user-level)
 *
 * This allows external projects to use IRIS Prime MCP servers with type safety.
 */
...
function generateIndex(serverNames) {
    const exports = serverNames.map(name => {
        ...
        const className = safeName.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join('');
        return `export { ${className}MCPWrapper } from './${safeName}';`;
    }).join('\n');
    return `
/**
 * IRIS Prime MCP Wrappers
