 ...
 * ```
 */
export async function getRecentLogs(expertId, limit = 50) {
    const supabase = getSupabase();
    const project = getProjectId();
    let query = supabase
        .from('model_run_log')
...
export async function detectDrift(expertId, version, options) {
    ...
    const baselineConfidence = baselineData.reduce((sum, r) => sum + r.confidence, 0) / baselineData.length;
    const recentConfidence = recentData.reduce((sum, r) => sum + r.confidence, 0) / recentData.length;
