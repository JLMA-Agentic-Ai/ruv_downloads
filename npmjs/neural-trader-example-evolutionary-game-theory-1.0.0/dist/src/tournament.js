...
export class Tournament {
     ...
     */
    calculateDiversityIndex(players) {
     ...
     */
    getMatchHistory(strategy1Id, strategy2Id) {
        const histories = [];
    ...
    analyzeStrategy(strategyId) {
        ...
        const performanceByOpponent = new Map();
        for (const opponent of Array.from(this.config.strategies)) {
            if (opponent.id === strategyId)
                continue;
            ...
            if (histories.length > 0) {
                ...
                performanceByOpponent.set(opponent.id, totalScore / (histories.length * this.config.roundsPerMatch));
            }
        }
        return {
            averageScore,
 ...
 */
export function quickTournament(strategies, game, rounds = 100) {
