 ...
 */
import { Tournament, PRISONERS_DILEMMA, ALWAYS_COOPERATE, ALWAYS_DEFECT, TIT_FOR_TAT, TIT_FOR_TWO_TATS, GRIM_TRIGGER, PAVLOV, ADAPTIVE, GRADUAL, createGenerousTitForTat, } from '../src/index.js';
...
result1.rankings.forEach((player, index) => {
    ...
    console.log(`${index + 1}\t${paddedName}\t${player.score.toFixed(1)}\t${player.wins}\t${player.losses}\t${player.draws}\t${(winRate * 100).toFixed(1)}%`);
});
...
result2.rankings.forEach((player, index) => {
    ...
    console.log(`${index + 1}\t${paddedName}\t${player.score.toFixed(1)}\t${(coopRate * 100).toFixed(1)}%`);
});
console.log('');
...
//# sourceMappingURL=tournament-evolution.js.map