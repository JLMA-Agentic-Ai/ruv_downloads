 ...
 */
import { SwarmEvolution, PRISONERS_DILEMMA, HAWK_DOVE, Tournament, TIT_FOR_TAT, PAVLOV, } from '../src/index.js';
...
for (let i = 0; i < 30; i++) {
    ...
    if (i % 5 === 0) {
        console.log(`Gen ${i}: Best Fitness = ${result.bestFitness.toFixed(3)}, ` +
...
[0, 5, 10, 15, 20, 25, 29].forEach((gen) => {
    ...
    const avgFitness = result.convergenceHistory.reduce((a, b) => a + b, 0) / result.convergenceHistory.length;
    console.log(`${gen}\t${result.bestFitness.toFixed(3)}\t\t${avgFitness.toFixed(3)}\t\t${result.populationDiversity.toFixed(3)}`);
});
console.log('\nObservations:');
...
compResult.rankings.forEach((player, index) => {
    ...
    console.log(`${index + 1}\t${paddedName}\t${player.score.toFixed(1)}\t${(winRate * 100).toFixed(1)}%`);
});
console.log('');
...
//# sourceMappingURL=swarm-learning.js.map