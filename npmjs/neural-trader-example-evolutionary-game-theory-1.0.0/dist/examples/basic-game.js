 ...
 */
import { PRISONERS_DILEMMA, HAWK_DOVE, STAG_HUNT, ReplicatorDynamics, ESSCalculator, findAllESS, } from '../src/index.js';
...
console.log('Pure Strategy ESS:', pdESS.pure.map((s) => PRISONERS_DILEMMA.strategyNames[s]));
...
[0, 5, 10, 15, 20].forEach((gen) => {
    ...
    console.log(`${gen}\t${state.frequencies[0].toFixed(4)}\t\t${state.frequencies[1].toFixed(4)}\t\t${(state.averageFitness || 0).toFixed(4)}`);
});
console.log('\nConvergence:');
const finalState = dynamics.simulateUntilConvergence();
console.log(`Final: ${(finalState.frequencies[0] * 100).toFixed(2)}% Cooperate, ` +
...
[0, 10, 20, 30].forEach((gen) => {
    ...
    console.log(`${gen}\t${(state.frequencies[0] * 100).toFixed(1)}%\t${(state.frequencies[1] * 100).toFixed(1)}%`);
});
const hdESS = findAllESS(HAWK_DOVE);
console.log('\nESS Analysis:');
console.log('Pure ESS:', hdESS.pure.map((s) => HAWK_DOVE.strategyNames[s]));
console.log('Mixed ESS:', hdESS.mixed.map((ess) => `${(ess.strategy[0] * 100).toFixed(1)}% Dove, ${(ess.strategy[1] * 100).toFixed(1)}% Hawk`));
console.log('');
...
console.log(`${(shFinal.frequencies[0] * 100).toFixed(1)}% Stag, ` +
...
//# sourceMappingURL=basic-game.js.map