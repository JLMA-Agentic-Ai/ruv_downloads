...
__export(index_exports, {
  APIError: () => APIError,
  AgenticSynth: () => AgenticSynth,
  BaseGenerator: () => BaseGenerator,
  CacheError: () => CacheError,
  CacheManager: () => CacheManager,
  CacheStore: () => CacheStore,
  CacheStrategySchema: () => CacheStrategySchema,
  DataTypeSchema: () => DataTypeSchema,
  EventGenerator: () => EventGenerator,
  EventOptionsSchema: () => EventOptionsSchema,
  GeneratorOptionsSchema: () => GeneratorOptionsSchema,
  MemoryCache: () => MemoryCache,
  ModelProviderSchema: () => ModelProviderSchema,
  ...
  TimeSeriesOptionsSchema: () => TimeSeriesOptionsSchema,
...
var BaseGenerator = class {
  ...
  constructor(config) {
    ...
    if (config.provider === "gemini" && geminiKey) {
      this.gemini = new import_generative_ai.GoogleGenerativeAI(geminiKey);
    }
  }
  /**
...
var AgenticSynth = class {
   ...
   */
  async generateTimeSeries(options = {}) {
    return this.timeSeriesGen.generate(options);
  }
  /**
   * Generate event data
   */
  async generateEvents(options = {}) {
    return this.eventGen.generate(options);
  }
  /**
   * Generate structured data
   */
  async generateStructured(options = {}) {
    return this.structuredGen.generate(options);
  }
  /**
  ...
  async generate(type, options = {}) {
    switch (type) {
      ...
      case "events":
        return this.generateEvents(options);
      case "structured":
      case "json":
        return this.generateStructured(options);
      default:
   ...
   */
  async *generateStream(type, options = {}) {
    const generator = this.getGenerator(type);
    yield* generator.generateStream(options);
  }
  /**
   * Generate multiple batches in parallel
   */
  async generateBatch(type, batchOptions, concurrency = 3) {
    const generator = this.getGenerator(type);
    return generator.generateBatch(batchOptions, concurrency);
  }
  /**
...
var index_default = AgenticSynth;
