...
import { ArrivalForecaster } from './arrival-forecaster.js';
...
export class HealthcareOptimizer {
     ...
     */
    async trainForecaster(historicalData) {
     ...
     */
    async optimize(startDate = new Date()) {
        ...
        const swarmResult = await this.swarm.optimize(forecasts, this.config.constraints, this.config.optimization, {
            planningHorizonDays: 7,
            shiftDuration: 8,
            costPerConstraintViolation: 1000
        }, startDate);
        ...
        const qualityScore = this.calculateQualityScore(swarmResult.bestSolution, queueMetrics);
     ...
     */
    async simulateQueue(forecasts, shifts) {
     ...
     * Calculate overall quality score
     */
    calculateQualityScore(solution, metrics) {
     ...
     */
    async updateWithActuals(timestamp, actualArrivals, actualWaitTime) {
