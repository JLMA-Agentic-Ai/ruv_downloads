...
export class SwarmCoordinator {
    ...
    agents;
    bestSolution;
    constructor(config, agentdbPath) {
        this.config = config;
        this.agents = [];
     ...
     */
    async optimize(forecasts, constraints, objective, schedulerConfig, startDate) {
        ...
        while (iteration < this.config.maxIterations && stableIterations < 10) {
            ...
            await this.evaluatePopulation(forecasts, constraints, objective, schedulerConfig, startDate);
            // Update best solution
            const currentBest = this.agents.reduce((best, agent) => agent.fitness > best.fitness ? agent : best);
     ...
     */
    async initializePopulation(schedulerConfig) {
        this.agents = [];
        const strategies = [
            'greedy_cost',
            'greedy_coverage',
     ...
     * Evaluate all agents in parallel
     */
    async evaluatePopulation(forecasts, constraints, objective, schedulerConfig, startDate) {
     ...
     */
    applyStrategy(baseConfig, strategy, explorationRadius) {
     ...
     */
    calculateFitness(solution, objective) {
        ...
        const fitness = objective.minimizeCost * costScore +
     ...
     */
    async evolvePopulation(objective) {
    ...
    generateSyntheticStaff() {
        const staff = [];
        const roles = ['physician', 'nurse', 'technician', 'specialist'];
        for (let i = 0; i < 50; i++) {
            ...
            staff.push({
                ...
                shiftPreference: ['day', 'evening', 'night', 'any'][Math.floor(Math.random() * 4)],
