...
exports.ElasticityLearner = void 0;
const agentdb_1 = require("agentdb");
class ElasticityLearner {
    constructor(dbPath = './data/elasticity.db') {
        this.db = new agentdb_1.AgentDB({ path: dbPath });
        this.observations = [];
     ...
     */
    async observe(price, demand, context) {
        const observation = {
            price,
            demand,
            context,
    ...
    async updateElasticity() {
        ...
        for (let i = 1; i < recentObs.length; i++) {
            ...
            if (prev.price === curr.price || prev.demand === 0 || curr.demand === 0)
                continue;
     ...
     */
    getElasticity(context) {
        // Load from DB or use default
        const stored = this.db.get('current_elasticity');
     ...
     */
    async getSegmentElasticity(segmentId) {
        ...
        if (segmentObs.length < 5) {
            return this.getElasticity({}); // Fallback to general
        }
     ...
     */
    async learnFromPurchase(purchase, segmentId) {
     ...
     */
    predictDemand(price, basePrice, baseDemand, context) {
...
exports.ElasticityLearner = ElasticityLearner;
//# sourceMappingURL=elasticity-learner.js.map