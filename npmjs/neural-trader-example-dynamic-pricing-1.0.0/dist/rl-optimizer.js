...
class RLOptimizer {
     ...
     */
    selectAction(context, explore = true) {
     ...
     */
    selectQLearningAction(state) {
        const stateKey = this.stateToKey(state);
        const qValues = this.qTable.get(stateKey);
        if (!qValues) {
            // Initialize Q-values for new state
            const newQValues = new Map();
     ...
     * SARSA update
     */
    updateSARSA(experience, nextContext) {
        const stateKey = this.stateToKey(experience.state);
        const nextStateKey = this.stateToKey(experience.nextState);
        if (!this.qTable.has(stateKey)) {
            const qValues = new Map();
        ...
        const newQ = currentQ + this.config.learningRate * (experience.reward + this.config.discountFactor * nextQ - currentQ);
        qValues.set(experience.action.index, newQ);
    }
    /**
    ...
    updateActorCritic(experience) {
        ...
        // Update value
        this.valueNetwork.set(stateKey, currentValue + this.config.learningRate * tdError);
...
exports.RLOptimizer = RLOptimizer;
//# sourceMappingURL=rl-optimizer.js.map