...
import { logger, loadConfig, updateConfig, checkToolInstalled, installTool } from '../utils/index.js';
...
async function installTools(toolNames, options) {
    ...
    for (const name of toolNames) {
        const tool = AVAILABLE_TOOLS.find(t => t.name === name || t.displayName.toLowerCase() === name.toLowerCase());
        if (!tool) {
            if (options.json) {
