...
import { HACKATHON_NAME, TRACKS, AVAILABLE_TOOLS, DISCORD_URL, WEBSITE_URL, GITHUB_URL } from '../constants.js';
...
function showPackagesHelp() {
    ...
    logger.divider();
    const npmTools = AVAILABLE_TOOLS.filter(t => t.installCommand.includes('npm') || t.installCommand.includes('npx'));
  ...
  ${chalk.bold.cyan(tool.displayName)} (${chalk.gray(tool.name)})
    ...
    console.log(chalk.bold('\n\nOTHER INSTALLATIONS'));
    logger.divider();
    const otherTools = AVAILABLE_TOOLS.filter(t => !t.installCommand.includes('npm') &&
