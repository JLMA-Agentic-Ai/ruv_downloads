...
function validateCommand(command) {
    ...
    const isAllowed = ALLOWED_COMMAND_PREFIXES.some(prefix => baseCommand === prefix || baseCommand.startsWith(`${prefix}/`));
    if (!isAllowed) {
    ...
    for (const pattern of dangerousPatterns) {
        if (pattern.test(command)) {
            throw new Error(`Command contains potentially unsafe characters: ${command}`);
        }
    }
}
/**
