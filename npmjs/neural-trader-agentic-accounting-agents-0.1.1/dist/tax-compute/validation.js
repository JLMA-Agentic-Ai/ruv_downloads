...
exports.TaxInputValidator = exports.ValidationError = void 0;
...
class TaxInputValidator {
    ...
    validateTransaction(tx) {
        ...
        if (!['BUY', 'SELL'].includes(tx.transactionType)) {
            throw new ValidationError('Transaction type must be BUY or SELL', 'transactionType', 'INVALID_TYPE');
        }
        // Numeric fields
    ...
    validateTaxLot(lot) {
        ...
        if (parseFloat(lot.remainingQuantity) < 0) {
            throw new ValidationError('Remaining quantity cannot be negative', 'remainingQuantity', 'INVALID_VALUE');
        }
        if (parseFloat(lot.remainingQuantity) > parseFloat(lot.quantity)) {
            throw new ValidationError('Remaining quantity cannot exceed total quantity', 'remainingQuantity', 'INVALID_VALUE');
        }
     ...
     */
    validateSaleAndLots(sale, lots) {
        // Must be a SELL transaction
        if (sale.transactionType !== 'SELL') {
            throw new ValidationError('Transaction must be a SELL type', 'transactionType', 'INVALID_TYPE');
        }
        // Must have lots
        if (lots.length === 0) {
        ...
        if (mismatchedLots.length > 0) {
            throw new ValidationError(`All lots must be for asset ${sale.asset}`, 'asset', 'ASSET_MISMATCH');
        }
        ...
        if (parseFloat(totalAvailable) < parseFloat(sale.quantity)) {
            throw new ValidationError(`Insufficient quantity: need ${sale.quantity}, have ${totalAvailable}`, 'quantity', 'INSUFFICIENT_QUANTITY');
        }
        ...
        for (const lot of lots) {
            ...
            if (lotDate > saleDate) {
                throw new ValidationError(`Lot ${lot.id} acquired after sale date`, 'acquisitionDate', 'INVALID_DATE');
            }
        }
    }
    /**
    ...
    validateDecimal(value, field) {
        ...
        if (!decimalRegex.test(value)) {
            throw new ValidationError(`${field} must be a valid decimal string`, field, 'INVALID_FORMAT');
        }
    }
    /**
    ...
    validateDate(value, field) {
        ...
        if (isNaN(date.getTime())) {
            throw new ValidationError(`${field} must be a valid ISO 8601 date`, field, 'INVALID_DATE');
        }
        ...
        if (date < minDate) {
            throw new ValidationError(`${field} is before minimum date (2009-01-01)`, field, 'DATE_TOO_OLD');
        }
        if (date > maxDate) {
            throw new ValidationError(`${field} is in the future`, field, 'DATE_IN_FUTURE');
        }
    }
}
