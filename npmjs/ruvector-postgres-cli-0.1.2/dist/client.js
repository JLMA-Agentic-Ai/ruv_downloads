...
export class RuVectorClient {
    ...
    async sparseDistance(a, b, metric) {
        ...
        const result = await this.query(`SELECT ${funcMap[metric]}($1::sparsevec, $2::sparsevec) as distance`, [a, b]);
        return result[0].distance;
    }
    async sparseBM25(query, doc, docLen, avgDocLen, k1 = 1.2, b = 0.75) {
        const result = await this.query('SELECT ruvector_sparse_bm25($1::sparsevec, $2::sparsevec, $3, $4, $5, $6) as score', [query, doc, docLen, avgDocLen, k1, b]);
        return result[0].score;
    }
    async sparseTopK(sparse, k) {
        const originalNnz = await this.query('SELECT ruvector_sparse_nnz($1::sparsevec) as nnz', [sparse]);
        const result = await this.query('SELECT ruvector_sparse_top_k($1::sparsevec, $2)::text as result', [sparse, k]);
        const newNnzResult = await this.query('SELECT ruvector_sparse_nnz($1::sparsevec) as nnz', [result[0].result]);
        return {
    ...
    async sparsePrune(sparse, threshold) {
        const originalNnz = await this.query('SELECT ruvector_sparse_nnz($1::sparsevec) as nnz', [sparse]);
        const result = await this.query('SELECT ruvector_sparse_prune($1::sparsevec, $2)::text as result', [sparse, threshold]);
        const newNnzResult = await this.query('SELECT ruvector_sparse_nnz($1::sparsevec) as nnz', [result[0].result]);
        return {
            vector: result[0].result,
            nnz: newNnzResult[0].nnz,
    ...
    async denseToSparse(dense) {
        ...
        const nnzResult = await this.query('SELECT ruvector_sparse_nnz($1::sparsevec) as nnz', [result[0].result]);
        return {
            vector: result[0].result,
            nnz: nnzResult[0].nnz,
        };
    ...
    async sparseToDense(sparse) {
        const result = await this.query('SELECT ruvector_sparse_to_dense($1::sparsevec) as result', [sparse]);
    ...
    async sparseInfo(sparse) {
        ...
        ruvector_sparse_norm($1::sparsevec) as norm`, [sparse]);
    ...
    // ============================================================================
    async computeAttention(query, keys, values, type = 'scaled_dot') {
        let funcName;
        let params;
        if (type === 'multi_head') {
            ...
            const result = await this.query('SELECT ruvector_attention_scores($1::real[], $2::real[][], $3) as scores', [query, keys, 'scaled_dot']);
            return { output: result[0].scores };
        }
        const result = await this.query(`SELECT ${funcName}($1::real[], $2::real[][], $3::real[][], $4) as output`, params);
        return { output: result[0].output };
    }
    async listAttentionTypes() {
        const result = await this.query('SELECT name FROM ruvector_attention_types()');
