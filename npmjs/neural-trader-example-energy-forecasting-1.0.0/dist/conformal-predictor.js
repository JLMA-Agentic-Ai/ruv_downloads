...
exports.EnergyConformalPredictor = void 0;
 ...
 */
class EnergyConformalPredictor {
    predictor = null;
    implementationType = 'pure';
    isAdaptive;
    modelType;
    calibrationHistory = [];
    constructor(modelType, config = {}, adaptive = false) {
    ...
    async initialize() {
        if (this.isAdaptive) {
            const { predictor, type } = await (0, predictor_1.createAdaptivePredictor)({
                targetCoverage: 0.90,
                gamma: 0.02,
                implementation: 'auto',
                fallbackToPure: true
            ...
            const { predictor, type } = await (0, predictor_1.createPredictor)({
                alpha: 0.1,
                implementation: 'auto',
                fallbackToPure: true
            });
     ...
     */
    async calibrate(historicalData, modelPredictions) {
        if (!this.predictor) {
     ...
     */
    async forecast(pointPrediction, timestamp, actualValue) {
        if (!this.predictor) {
            throw new Error('Predictor not initialized. Call initialize() first.');
        }
        let interval;
        if (this.isAdaptive && actualValue !== undefined) {
            ...
            const adaptivePredictor = this.predictor;
     ...
     */
    async update(prediction, actual, timestamp) {
        if (!this.predictor) {
     ...
     */
    computeCoverage(predictions, actuals) {
        if (!this.predictor) {
            return 0;
        }
        let covered = 0;
...
exports.EnergyConformalPredictor = EnergyConformalPredictor;
//# sourceMappingURL=conformal-predictor.js.map