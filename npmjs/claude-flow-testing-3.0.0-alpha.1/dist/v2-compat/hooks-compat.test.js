...
import { V2CompatibilityValidator, V2_HOOKS, } from './compatibility-validator.js';
/**
 * Create mock hooks system
 */
function createMockHooksSystem() {
    const v3Hooks = V2_HOOKS.map(h => h.v3Equivalent || h.name);
    const registeredHooks = new Set(v3Hooks);
    return {
        trigger: vi.fn().mockImplementation(async (name, params) => {
        ...
        isRegistered: vi.fn().mockImplementation((name) => registeredHooks.has(name)),
        getHookInfo: vi.fn().mockImplementation((name) => {
...
describe('V2 Hooks Compatibility', () => {
    ...
    describe('Edit Hooks', () => {
        const editHooks = V2_HOOKS.filter(h => h.name.includes('edit') || h.name.includes('create'));
        it.each(editHooks)('should support V2 hook: $name', async (hook) => {
            const params = Object.fromEntries(hook.parameters.map(p => [p, 'test-value']));
    ...
    describe('Command Hooks', () => {
        const commandHooks = V2_HOOKS.filter(h => h.name.includes('command') || h.name.includes('bash'));
        it.each(commandHooks)('should support V2 hook: $name', async (hook) => {
            const params = Object.fromEntries(hook.parameters.map(p => [p, 'test-value']));
            const result = await mockHooks.trigger(hook.v3Equivalent || hook.name, params);
            expect(result.handled).toBe(true);
        });
    ...
    describe('Task Hooks', () => {
        ...
        it.each(taskHooks)('should support V2 hook: $name', async (hook) => {
            const params = Object.fromEntries(hook.parameters.map(p => [p, p === 'task' ? { id: 'task-1' } : 'test']));
            const result = await mockHooks.trigger(hook.v3Equivalent || hook.name, params);
            expect(result.handled).toBe(true);
        });
    ...
    describe('Agent Hooks', () => {
        ...
        it.each(agentHooks)('should support V2 hook: $name', async (hook) => {
            const params = Object.fromEntries(hook.parameters.map(p => [p, 'test-value']));
            const result = await mockHooks.trigger(hook.v3Equivalent || hook.name, params);
            expect(result.handled).toBe(true);
        });
    ...
    describe('Swarm Hooks', () => {
        ...
        it.each(swarmHooks)('should support V2 hook: $name', async (hook) => {
            const params = Object.fromEntries(hook.parameters.map(p => [p, 'test-value']));
            const result = await mockHooks.trigger(hook.v3Equivalent || hook.name, params);
            expect(result.handled).toBe(true);
        });
    ...
    describe('Memory Hooks', () => {
        ...
        it.each(memoryHooks)('should support V2 hook: $name', async (hook) => {
            const params = Object.fromEntries(hook.parameters.map(p => [p, p === 'entry' ? { id: 'mem-1' } : 'test']));
            const result = await mockHooks.trigger(hook.v3Equivalent || hook.name, params);
            expect(result.handled).toBe(true);
        });
    ...
    describe('Learning Hooks', () => {
        ...
        it.each(learningHooks)('should support V2 hook: $name', async (hook) => {
            const params = Object.fromEntries(hook.parameters.map(p => [p, 'test-value']));
            const result = await mockHooks.trigger(hook.v3Equivalent || hook.name, params);
            expect(result.handled).toBe(true);
        });
    ...
    describe('Session Hooks', () => {
        ...
        it.each(sessionHooks)('should support V2 hook: $name', async (hook) => {
            const params = Object.fromEntries(hook.parameters.map(p => [p, 'test-value']));
            const result = await mockHooks.trigger(hook.v3Equivalent || hook.name, params);
            expect(result.handled).toBe(true);
        });
    ...
    describe('Security Hooks', () => {
        ...
        it.each(securityHooks)('should support V2 hook: $name', async (hook) => {
            const params = Object.fromEntries(hook.parameters.map(p => [p, 'test-value']));
            const result = await mockHooks.trigger(hook.v3Equivalent || hook.name, params);
            expect(result.handled).toBe(true);
        });
    ...
    describe('Full Hooks Validation', () => {
        ...
        it('should detect all 42 V2 hooks', async () => {
            const result = await validator.validateHooks();
            const hookChecks = result.checks.filter(c => c.name.startsWith('Hook:') && !c.name.includes('Param') && !c.name.includes('Return'));
...
//# sourceMappingURL=hooks-compat.test.js.map