...
export class AdvancedReasoningEngine {
     ...
     */
    async wasmPredict(action, state) {
        try {
            const stateVector = this.stateToVector(state);
    ...
    fallbackAnalyze(state, goal) {
        ...
        if (facets.length > 1)
            suggestedActions.push('parallel_research');
        if (domains.includes('academic'))
            suggestedActions.push('academic_search');
    ...
    detectDomains(query) {
        const domains = [];
        if (/AI|artificial intelligence|machine learning/i.test(query))
            domains.push('ai');
        if (/quantum|physics/i.test(query))
            domains.push('physics');
        if (/crypto|security|encryption/i.test(query))
            domains.push('security');
        if (/research|academic|paper/i.test(query))
            domains.push('academic');
        if (/latest|recent|2024|2025/i.test(query))
            domains.push('recent');
        return domains;
    }
    detectTemporalRequirements(query) {
        if (/latest|recent|newest/i.test(query))
            return 'recent developments';
        if (/2024|2025/i.test(query))
            return 'specific timeframe';
        if (/breakthrough|advance/i.test(query))
            return 'emerging trends';
        if (/future|prediction/i.test(query))
            return 'predictive analysis';
        return null;
    }
    detectQueryFacets(query) {
        const facets = [];
        if (/breakthrough|development/i.test(query))
            facets.push('technological advances');
        if (/compare|comparison/i.test(query))
            facets.push('comparative analysis');
        if (/implications|impact/i.test(query))
            facets.push('impact assessment');
        if (/capabilities/i.test(query))
            facets.push('capability analysis');
        return facets.length > 0 ? facets : ['general inquiry'];
    }
    calculateConfidence(complexity, facetCount, domainCount) {
        let confidence = 0.75;
        ...
        if (facetCount === 1)
            confidence += 0.1;
        else if (facetCount > 3)
            confidence -= 0.15;
        if (domainCount > 0 && domainCount <= 2)
            confidence += 0.1;
     ...
     */
    interpretPredictions(predictions, goal) {
...
//# sourceMappingURL=advanced-reasoning-engine.js.map