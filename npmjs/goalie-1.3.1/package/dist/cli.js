...
program
    ...
    try {
        ...
        const domains = options.domains ? options.domains.split(',').map((d) => d.trim()) : undefined;
        const trustedIssuers = options.trustedIssuers
            ? options.trustedIssuers.split(',').map((i) => i.trim())
...
reasoningCmd
    ...
    try {
        ...
        const citations = options.citations
            ? options.citations.split(',').map((c) => c.trim())
            : [];
