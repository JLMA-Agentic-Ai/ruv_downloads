...
export class AntiHallucinationPlugin {
    ...
    citationRequirement = 0.8; // 80% of claims must have citations
     ...
     */
    assessHallucinationRisk(claims) {
     ...
     */
    isClaimGrounded(claim, citations) {
        const claimKeywords = this.extractKeywords(claim);
        for (const citation of citations) {
            const overlap = this.calculateKeywordOverlap(claimKeywords, this.extractKeywords(citation));
            if (overlap > 0.3)
                return true;
        }
        return false;
     ...
     */
    addUncertaintyMarker(content, claim) {
     ...
     */
    checkForCriticalUnverifiedClaims(result) {
        ...
        if (hasCritical) {
            ...
            return criticalClaims.some(claim => !this.isClaimGrounded(claim, result.citations || []));
        }
        return false;
    }
    /**
     ...
     */
    assessCitationQuality(citations) {
        ...
        for (const citation of citations) {
            const hasQualityDomain = qualityDomains.some(domain => citation.toLowerCase().includes(domain));
            if (hasQualityDomain)
                qualityScore += 1;
        }
    ...
    async execute(params) {
        ...
        for (const claim of claims) {
            ...
            const supportingCitations = this.findSupportingCitations(claim, [
                ...providedCitations,
                ...searchCitations.map((r) => `${r.title}: ${r.snippet}`)
            ]);
            ...
            const verificationResponse = await client.chat({
                messages: [
                    ...
                    {
                        role: 'user',
                        content: `Claim: ${claim}\n\nEvidence:\n${searchCitations.slice(0, 3).map((r, i) => `[${i + 1}] ${r.title}: ${r.snippet?.substring(0, 200) || ''}`).join('\n')}\n\nIs this claim verified?`
                    }
                ],
                model: 'sonar',
                temperature: 0.1,
        ...
        const overallVerification = verifiedClaims.filter((c) => c.verified).length / claims.length;
        return {
            success: true,
            ...
            reasoning: `Anti-Hallucination Analysis:\n\n` +
                `Verified ${verifiedClaims.filter((c) => c.verified).length}/${claims.length} claims with citations.\n` +
                `Overall Verification Score: ${(overallVerification * 100).toFixed(1)}%\n\n` +
                `${verifiedClaims.filter((c) => !c.verified).length > 0 ?
                    `⚠️ Warning: ${verifiedClaims.filter((c) => !c.verified).length} claims lack supporting evidence\n` :
     ...
     */
    findSupportingCitations(claim, citations) {
...
//# sourceMappingURL=anti-hallucination-plugin.js.map