...
export class AgenticResearchFlowPlugin {
     ...
     */
    async executeExplorationPhase(query) {
     ...
     */
    async executeValidationPhase(searchResults) {
        const validators = this.agents.filter(a => a.role === 'validator' || a.role === 'fact-checker');
        const validationResults = await Promise.all(validators.map(async (agent) => {
            agent.status = 'working';
            ...
            if (validation.criticalIssues) {
                this.researchFlow.criticalFindings.push(...validation.criticalIssues);
            }
            return validation;
        }));
        return {
     ...
     */
    async executeSynthesisPhase(searchResults) {
        const synthesizers = this.agents.filter(a => a.role === 'synthesizer');
        const synthesisResults = await Promise.all(synthesizers.map(async (agent) => {
            agent.status = 'working';
     ...
     */
    async executeCritiquePhase(synthesisResults) {
        const critics = this.agents.filter(a => a.role === 'critic');
        const critiqueResults = await Promise.all(critics.map(async (agent) => {
            agent.status = 'working';
            // Critical analysis
     ...
     */
    async executeAgentWork(agent, query) {
     ...
     */
    performCritique(synthesisResults) {
        const issues = [];
     ...
     */
    async generateAgentFindings(agentType, query) {
        const client = this.getClient();
        const agentPrompts = {
     ...
     */
    synthesizeFindingsString(agentResults) {
        const findings = agentResults.map((r) => r.findings).join(' ');
        return `Based on multi-agent analysis: ${findings.substring(0, 200)}...`;
    }
}
export default new AgenticResearchFlowPlugin();
//# sourceMappingURL=agentic-research-flow-plugin.js.map