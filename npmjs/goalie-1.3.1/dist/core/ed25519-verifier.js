...
export class Ed25519Verifier {
     ...
     */
    registerTrustedKey(keyId, publicKeyBase64) {
...
export class AntiHallucinationVerifier {
     ...
     */
    async verifyClaims(claims, citations) {
     ...
     */
    async extractAndVerifyFacts(text, citations) {
     ...
     */
    async signSearchResult(searchResult) {
        if (!this.ed25519Verifier.config.signResult) {
            return searchResult;
        }
        // Sign the main answer
        const answerSig = await this.ed25519Verifier.sign(searchResult.answer || JSON.stringify(searchResult));
        return {
            ...searchResult,
