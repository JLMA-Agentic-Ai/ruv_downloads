 ...
 */
export class StateOfArtAntiHallucination {
     ...
     */
    async verifyRAGGrounding(content, citations) {
        const issues = [];
        const suggestions = [];
        ...
        for (const pattern of factualPatterns) {
            ...
            for (const claim of matches) {
                ...
                const grounded = citations.some(citation => {
                    ...
                    const matchedTerms = keyTerms.filter(term => snippet.includes(term) || title.includes(term));
     ...
     */
    async verifyConsistency(content, alternativeResponses) {
        const issues = [];
        const suggestions = [];
     ...
     */
    async verifyUncertaintyCalibration(content) {
        const issues = [];
        const suggestions = [];
     ...
     */
    async verifyMetamorphicStability(content, originalQuery) {
        const issues = [];
        const suggestions = [];
     ...
     */
    async verifyCitationAttribution(content, citations) {
        const issues = [];
        const suggestions = [];
        ...
        for (const pattern of citationRequiredPatterns) {
            ...
            for (const match of matches) {
                ...
                const nearbyText = content.substring(Math.max(0, matchIndex - 50), Math.min(content.length, matchIndex + 150));
    ...
    detectContradiction(sent1, sent2) {
        ...
        for (const [word1, word2] of opposites) {
            if ((s1Lower.includes(word1) && s2Lower.includes(word2)) ||
                ...
                const sharedWords = s1Lower.split(/\s+/).filter(w => s2Lower.includes(w) && w.length > 4);
     ...
     */
    async performComprehensiveValidation(state) {
    ...
    hooks = {
        ...
        afterSynthesize: async (result) => {
            const validation = await this.performComprehensiveValidation(result.state || result);
        ...
        onReplan: async (failedStep, state) => {
            console.log('\n🛡️ ANTI-HALLUCINATION REPLAN TRIGGERED');
            const validation = state.hallucination_validation;
            if (validation?.detailedResults) {
                console.log('\nFailed Checks:');
                Object.entries(validation.detailedResults).forEach(([check, result]) => {
                    if (!result.valid) {
...
export function createAntiHallucinationPlugin() {
    return new StateOfArtAntiHallucination();
}
// Export default instance
export default new StateOfArtAntiHallucination();
//# sourceMappingURL=state-of-art-anti-hallucination.js.map