...
export class ChainOfThoughtPlugin {
    ...
    hooks = {
        ...
        afterSearch: async (results, context) => {
            ...
            const bestPath = validatedPaths.reduce((best, current) => current.score > best.score ? current : best);
     ...
     */
    async generateThoughtTree(query) {
     ...
     */
    extractReasoningPaths(node, currentPath = []) {
    ...
    validatePath(path, results) {
        ...
        for (const node of path) {
            ...
            const supportCount = thoughtWords.filter(word => resultText.includes(word)).length;
    ...
    async execute(params) {
        ...
        // Select best path
        const bestPath = pathAnalysis.reduce((best, current) => current.totalConfidence > best.totalConfidence ? current : best);
        // Generate reasoning explanation
        const reasoningSteps = bestPath.steps.map((step, i) => `  ${i + 1}. ${step.thought} (confidence: ${(step.confidence * 100).toFixed(1)}%)`).join('\n');
        return {
            success: true,
            method: 'chain-of-thought',
            query,
...
//# sourceMappingURL=chain-of-thought-plugin.js.map