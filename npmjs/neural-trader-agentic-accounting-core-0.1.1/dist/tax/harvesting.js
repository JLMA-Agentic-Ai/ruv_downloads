...
exports.TaxLossHarvestingService = void 0;
const logger_1 = require("../utils/logger");
const decimal_js_1 = __importDefault(require("decimal.js"));
class TaxLossHarvestingService {
    washSalePeriod = 30; // 30 days before and after
    /**
     * Scan portfolio for harvesting opportunities
     * Performance target: Identify 95%+ harvestable losses
     */
    async scanOpportunities(positions, currentPrices, recentTransactions, taxRate = 0.35) {
        logger_1.logger.info(`Scanning ${positions.length} positions for harvesting opportunities`);
        const opportunities = [];
        for (const position of positions) {
            ...
            const recommendation = this.determineRecommendation(unrealizedLoss, washSaleCheck, potentialTaxSavings);
            opportunities.push({
        ...
        logger_1.logger.info(`Found ${opportunities.length} harvesting opportunities`, {
            harvestable: opportunities.filter(o => o.recommendation === 'HARVEST').length,
            totalPotentialSavings: opportunities
                .reduce((sum, o) => sum.add(o.potentialTaxSavings), new decimal_js_1.default(0))
                .toString()
        });
    ...
    async checkWashSale(asset, recentTransactions) {
        ...
        const recentBuys = recentTransactions.filter(tx => tx.asset === asset &&
            tx.type === 'BUY' &&
        ...
        let daysUntilSafe = 0;
        if (hasViolation) {
            const mostRecentBuy = recentBuys.reduce((latest, tx) => tx.timestamp > latest.timestamp ? tx : latest);
            const safeDate = new Date(mostRecentBuy.timestamp.getTime() + this.washSalePeriod * 24 * 60 * 60 * 1000);
     ...
     */
    async findReplacementAssets(asset, correlationThreshold = 0.7) {
        logger_1.logger.info(`Finding replacement assets for ${asset}`);
    ...
    rankOpportunities(opportunities) {
        return opportunities.sort((a, b) => {
            ...
            if (a.washSaleRisk && !b.washSaleRisk)
                return 1;
            if (!a.washSaleRisk && b.washSaleRisk)
                return -1;
            return 0;
     ...
     */
    determineRecommendation(unrealizedLoss, washSaleCheck, potentialTaxSavings) {
     ...
     */
    calculateExpirationDate(washSaleCheck) {
        if (!washSaleCheck.hasViolation) {
            return undefined;
        }
        const mostRecentBuy = washSaleCheck.recentBuys.reduce((latest, tx) => tx.timestamp > latest.timestamp ? tx : latest);
        return new Date(mostRecentBuy.timestamp.getTime() + this.washSalePeriod * 24 * 60 * 60 * 1000);
    }
    /**
...
exports.TaxLossHarvestingService = TaxLossHarvestingService;
