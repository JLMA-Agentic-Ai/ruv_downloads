...
export class QuantumAnnealer {
     ...
     */
    quantumMonteCarloStep(temperature, transverseField) {
     ...
     */
    trotterSliceUpdate(temperature, transverseField, slice, numSlices) {
        const n = this.config.numQubits;
        for (let i = 0; i < n; i++) {
            const newState = [...this.currentState];
     ...
     */
    pathIntegralStep(temperature, transverseField) {
     ...
     */
    computeQuantumFluctuations(transverseField) {
     ...
     */
    checkConvergence() {
        if (this.annealingPath.length < 10)
            return false;
     ...
     */
    estimateSuccessProbability(energy) {
...
export class QUBOFormulator {
     ...
     * Convert TSP to QUBO
     */
    static tspToQUBO(distanceMatrix) {
     ...
     */
    static constraintSatisfactionToQUBO(numVars, constraints) {
        const Q = Array(numVars).fill(0).map(() => Array(numVars).fill(0));
     ...
     */
    static portfolioToQUBO(returns, covarianceMatrix, budget, riskAversion) {
 ...
 */
export async function solveQUBO(qubo, config = {}) {
 ...
 */
export async function solveMaxCutAnnealing(edges, config = {}) {
 ...
 */
export async function solveTSPAnnealing(distanceMatrix, config = {}) {
    const qubo = QUBOFormulator.tspToQUBO(distanceMatrix);
    return solveQUBO(qubo, config);
}
//# sourceMappingURL=quantum-annealing.js.map