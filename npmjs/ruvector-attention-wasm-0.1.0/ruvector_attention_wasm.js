 ...
 * @param {any} keys
 * @param {any} values
 * @param {number | null} [_scale]
 * @returns {Float32Array}
 */
export function scaled_dot_attention(query, keys, values, _scale) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        const ptr0 = passArrayF32ToWasm0(query, wasm.__wbindgen_export);
        const len0 = WASM_VECTOR_LEN;
        wasm.scaled_dot_attention(retptr, ptr0, len0, addHeapObject(keys), addHeapObject(values), isLikeNone(_scale) ? 0x100000001 : Math.fround(_scale));
        var r0 = getDataViewMemory0().getInt32(retptr + 4 * 0, true);
        var r1 = getDataViewMemory0().getInt32(retptr + 4 * 1, true);
        var r2 = getDataViewMemory0().getInt32(retptr + 4 * 2, true);
        var r3 = getDataViewMemory0().getInt32(retptr + 4 * 3, true);
