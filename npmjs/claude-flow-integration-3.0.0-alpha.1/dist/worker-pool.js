...
import { SpecializedWorker } from './specialized-worker.js';
import { LongRunningWorker } from './long-running-worker.js';
/**
...
export class WorkerPool extends EventEmitter {
    ...
    async initialize() {
        ...
        const initPromises = Array.from(this.workers.values()).map((worker) => worker.initialize().catch((error) => {
    ...
    async shutdown() {
        ...
        const shutdownPromises = Array.from(this.workers.values()).map((worker) => worker.shutdown().catch((error) => {
    ...
    spawn(config, options = {}) {
        ...
        if (this.workers.size >= this.config.maxWorkers && !options.replace) {
            throw new Error(`Pool ${this.id} at maximum capacity (${this.config.maxWorkers} workers)`);
        }
        // Handle replacement
        ...
        let worker;
        if ('domain' in config) {
            worker = new SpecializedWorker(config);
        }
        else if ('checkpointInterval' in config) {
            worker = new LongRunningWorker(config);
        }
        else {
     ...
     */
    routeTaskWithDetails(task, topK = 1) {
    ...
    balanceLoad() {
        ...
        else if (stats.avgUtilization < this.config.scaleDownThreshold &&
    ...
    getWorkersByCapability(capability) {
        return Array.from(this.workers.values()).filter((worker) => worker.capabilities.includes(capability));
    }
    /**
     * Get pool statistics
     */
    getStats() {
        ...
        const busyWorkers = workers.filter((w) => w.status === 'busy' || w.load > 0.9);
        const unhealthyWorkers = workers.filter((w) => w.getHealth().status === 'unhealthy');
        // Calculate averages
        ...
        const avgHealthScore = workers.length > 0
            ...
            : 1;
        const avgTaskDuration = this.poolMetrics.tasksProcessed > 0
    ...
    extractRequiredCapabilities(task) {
        if (task.metadata?.requiredCapabilities) {
            return task.metadata.requiredCapabilities;
        }
        return [];
    }
    /**
 ...
 */
export async function createAndInitializeWorkerPool(config = {}) {
