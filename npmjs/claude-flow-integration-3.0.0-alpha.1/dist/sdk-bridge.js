...
export class SDKBridge extends EventEmitter {
    ...
    async initialize() {
        ...
        try {
            ...
            if (!compatibility.compatible) {
                throw new Error(`SDK version ${this.currentVersion.full} is not compatible. ` +
    ...
    async checkCompatibility() {
        ...
        const compatible = this.compareVersions(current, minVersion) >= 0 &&
     ...
     */
    translateDeprecatedAPI(oldAPI, args) {
     ...
     */
    async wrapAPICall(apiName, apiCall, fallback) {
     ...
     */
    getFeatureRequirements(capability) {
     ...
     */
    async negotiateVersion(preferredVersion) {
    ...
    generateMigrationExample(oldAPI, newAPI) {
 ...
 */
export async function createSDKBridge(config) {
