...
export class SpecializedWorker extends WorkerBase {
    ...
    matchTask(task) {
        ...
        const { matched, missing, score: capabilityScore } = this.calculateCapabilityMatch(requiredCapabilities);
        ...
        const overallScore = capabilityScore * 0.3 +
            domainScore * 0.25 +
            embeddingScore * 0.25 +
            skillScore * 0.2;
        // Generate recommendations
        const recommendations = this.generateRecommendations(missing, taskDomain, overallScore);
        return {
            score: overallScore,
            breakdown: {
                capabilityScore,
     ...
     */
    async processTaskForDomain(task) {
    ...
    extractRequiredCapabilities(task) {
        ...
        if (task.metadata?.requiredCapabilities) {
            capabilities.push(...task.metadata.requiredCapabilities);
        }
     ...
     */
    inferCapabilitiesFromDescription(description) {
        const capabilities = [];
     ...
     */
    calculateCapabilityMatch(required) {
     ...
     */
    calculateDomainMatch(taskDomain) {
     ...
     */
    generateRecommendations(missing, taskDomain, score) {
 ...
 */
export function createSpecializedWorker(domain, config = {}) {
 ...
 */
export function createFrontendWorker(config = {}) {
 ...
 */
export function createBackendWorker(config = {}) {
 ...
 */
export function createTestingWorker(config = {}) {
...
//# sourceMappingURL=specialized-worker.js.map