...
use crate::models::ruvltra::{RuvLtraConfig, RuvLtraModel, QuantizationType};
...
impl KvCache {
    ...
    #[inline]
    pub fn get_or_create(&self, session_id: &str) -> dashmap::mapref::one::RefMut<'_, String, KvCacheEntry> {
        ...
        self.entries.entry(session_id.to_string())
    ...
    pub fn update_memory_usage(&self, old_size: usize, new_size: usize) {
        if new_size > old_size {
            self.current_memory_bytes.fetch_add((new_size - old_size) as u64, Ordering::Relaxed);
        } else if old_size > new_size {
            self.current_memory_bytes.fetch_sub((old_size - new_size) as u64, Ordering::Relaxed);
        }
    }
    ...
    pub fn evict_if_needed(&self) {
        ...
        self.current_memory_bytes.store(current as u64, Ordering::Relaxed);
        ...
        for key in to_remove {
            if let Some((_, entry)) = self.entries.remove(&key) {
                self.current_memory_bytes.fetch_sub(entry.memory_bytes() as u64, Ordering::Relaxed);
    ...
    pub fn stats(&self) -> KvCacheStats {
        ...
        KvCacheStats {
            ...
            hit_rate: if total_ops > 0 { hits as f32 / total_ops as f32 } else { 0.0 },
            total_ops,
        }
    }
}
...
impl RuvLtraRlmBackend {
    ...
    pub fn with_model(model: RuvLtraModel, tokenizer: RuvTokenizer, config: RuvLtraRlmConfig) -> Self {
        let kv_cache = KvCache::new(
            config.model_config.num_hidden_layers,
            config.kv_cache_memory_mb * 1024 * 1024,
        );
    ...
    fn autoregressive_generate(
        ...
        let mut kv_caches: Vec<(Vec<f32>, Vec<f32>)> = kv_entry.keys.iter()
        ...
        for i in 0..params.max_tokens {
            ...
            let positions: Vec<usize> = (start_pos + i..start_pos + i + current_ids.len()).collect();
    ...
    fn sample_token(&self, logits: &[f32], params: &GenerationParams) -> Result<u32> {
        ...
        let top_k = if params.top_k > 0 { params.top_k } else { probs.len() };
    ...
    fn pool_hidden_states(&self, hidden_states: &[f32], seq_len: usize) -> Vec<f32> {
        ...
        match self.config.embedding_pooling {
            EmbeddingPooling::Mean => {
                self.pool_mean_optimized(hidden_states, seq_len, hidden_size)
            }
            ...
            EmbeddingPooling::Max => {
                self.pool_max_optimized(hidden_states, seq_len, hidden_size)
            }
        }
    }
    ...
    #[inline]
    fn pool_mean_optimized(&self, hidden_states: &[f32], seq_len: usize, hidden_size: usize) -> Vec<f32> {
    ...
    #[inline]
    fn pool_max_optimized(&self, hidden_states: &[f32], seq_len: usize, hidden_size: usize) -> Vec<f32> {
...
impl LlmBackendTrait for RuvLtraRlmBackend {
    ...
    fn generate(&self, prompt: &str, params: &GenerationParams) -> Result<GenerationOutput> {
        ...
        // Generate
        let (generated_ids, finish_reason) = self.autoregressive_generate(&input_ids, params, None)?;
        ...
        // Update stats
        self.stats.total_tokens.fetch_add(completion_tokens as u64, Ordering::Relaxed);
        self.stats.total_latency_us.fetch_add(start.elapsed().as_micros() as u64, Ordering::Relaxed);
...
impl RuvLtraMemoryStore {
    ...
    pub fn with_integration(integration: RuvectorIntegration, backend: Arc<RuvLtraRlmBackend>) -> Self {
        Self {
            integration: Arc::new(integration),
            backend,
            memory_count: AtomicU64::new(0),
...
impl MemoryStore for RuvLtraMemoryStore {
    fn store(&self, text: &str, metadata: MemoryMetadata) -> Result<MemoryId> {
        ...
        let vec_meta = VectorMetadata {
            source: metadata.source.clone().unwrap_or_else(|| "user".to_string()),
        ...
        self.integration.add_vector(id.clone(), embedding, vec_meta)?;
    ...
    fn search(&self, query_embedding: &[f32], top_k: usize) -> Result<Vec<MemorySpan>> {
        ...
        Ok(results.into_iter().map(|r| MemorySpan {
            id: r.id,
            text: r.metadata.as_ref()
...
impl RuvLtraEnvironment {
    ...
    pub fn answer_query(
        ...
        let answer = if decomposition.needs_decomposition() && depth < config.max_depth - 1 {
            ...
            RlmAnswer {
                text: synthesized,
                confidence: sub_answers.iter().map(|a| a.confidence).sum::<f32>() / sub_answers.len() as f32,
...
impl QueryDecomposer {
    ...
    pub fn decompose(&self, query: &str, _context: &QueryContext) -> Result<QueryDecomposition> {
        ...
        if lower.contains(" and ") {
            ...
            if parts.len() >= 2 {
                return Ok(QueryDecomposition {
                    strategy: DecompositionStrategy::Conjunction(parts.iter().map(|s| s.to_string()).collect()),
                    sub_queries: parts.iter().enumerate().map(|(i, q)| SubQuery {
        ...
        if aspect_count >= 2 {
            ...
            let aspect_queries: Vec<String> = aspects.iter()
            ...
            return Ok(QueryDecomposition {
                ...
                sub_queries: aspect_queries.iter().enumerate().map(|(i, q)| SubQuery {
                    query: q.clone(),
                    depends_on: Vec::new(),
