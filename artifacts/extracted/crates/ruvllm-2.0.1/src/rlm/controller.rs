...
use super::memory::{RlmMemory, MemoryConfig, MemorySearchResult};
...
use super::shared_types::{
    SharedText, SharedEmbedding, IntoSharedText, IntoSharedEmbedding,
...
impl<E: RlmEnvironment> RlmController<E> {
    ...
    pub async fn query(&self, input: &str) -> Result<QueryResult> {
        ...
        let sources: Vec<SourceAttribution> = context.iter().map(|r| {
        ...
        self.stats.total_tokens.fetch_add(result.usage.total_tokens as u64, Ordering::Relaxed);
    ...
    #[cfg(not(target_arch = "wasm32"))]
    pub async fn search_memory(&self, query: &str, top_k: usize) -> Result<Vec<MemorySearchResult>> {
    ...
    fn search_memory_internal(&self, embedding: &[f32], top_k: usize) -> Result<Vec<MemorySearchResult>> {
    ...
    fn cache_result_arc(&self, input: &str, result: Arc<QueryResult>) {
        ...
        {
            let mut cache = self.cache.write();
            cache.insert(hash_key, CachedResponse {
                result,
                cached_at: now,
                cached_at_secs: now.timestamp(),
            });
    ...
    fn update_avg_latency(&self, latency_us: u64) {
        ...
        if current == 0 {
            self.stats.avg_latency_us.store(latency_us, Ordering::Relaxed);
        } else {
            // EMA with alpha = 0.1
