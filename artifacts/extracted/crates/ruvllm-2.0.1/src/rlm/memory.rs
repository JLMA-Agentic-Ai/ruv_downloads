...
impl RlmMemory {
    ...
    fn search_hnsw(&self, hnsw: &HnswIndex, query_embedding: &[f32], top_k: usize) -> Result<Vec<MemorySearchResult>> {
        ...
        let hnsw_results = hnsw.search_with_ef(query_embedding, request_k, self.config.hnsw_ef_search)
    ...
    fn search_brute_force(&self, query_embedding: &[f32], top_k: usize) -> Result<Vec<MemorySearchResult>> {
    ...
    #[inline]
    fn search_topk_heap_batch(&self, query_embedding: &[f32], query_norm: f32, top_k: usize, threshold: f32) -> Result<Vec<MemorySearchResult>> {
    ...
    pub fn rebuild_index(&mut self) -> Result<()> {
        ...
        let mut new_index = HnswIndex::new(self.config.embedding_dim, DistanceMetric::Cosine, hnsw_config)
            .map_err(|e| RuvLLMError::Ruvector(format!("Failed to create HNSW index: {}", e)))?;

        // Re-insert all non-deleted entries
        for entry in self.entries.values() {
            new_index.add(entry.id.clone(), entry.embedding.clone())
                .map_err(|e| RuvLLMError::Ruvector(format!("Failed to add to HNSW: {}", e)))?;
        }
    ...
    pub fn cleanup_expired(&mut self) {
        ...
        let expired_ids: Vec<_> = self.entries.iter()
            .filter(|(_, entry)| {
...
mod tests {
    ...
    fn test_memory_list_pagination() {
        ...
        for i in 0..10 {
            memory.add(create_test_entry(&format!("{}", i), &format!("entry {}", i))).unwrap();
        }
    ...
    fn test_hnsw_memory_add_and_search() {
        ...
        assert!(results[0].score > 0.9, "Expected high similarity, got {}", results[0].score);
    }
    ...
    fn test_hnsw_memory_delete() {
        ...
        for result in &results {
            ...
            assert_ne!(result.id.as_str(), "entry-0", "Deleted entry-0 should not be returned");
            assert_ne!(result.id.as_str(), "entry-1", "Deleted entry-1 should not be returned");
            assert_ne!(result.id.as_str(), "entry-2", "Deleted entry-2 should not be returned");
        }
    }
