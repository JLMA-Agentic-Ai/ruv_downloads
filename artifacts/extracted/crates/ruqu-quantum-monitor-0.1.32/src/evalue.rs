...
impl EValueTest {
    ...
    pub fn update(&mut self, mmd_squared: f64) -> EValueUpdate {
        ...
        // Bound history size to prevent memory exhaustion (keep last 100k entries)
        const MAX_HISTORY_SIZE: usize = 100_000;
        if self.mmd_history.len() > MAX_HISTORY_SIZE {
            self.mmd_history.drain(0..MAX_HISTORY_SIZE / 2);
        }
        ...
        // Bound evalue history size
        if self.evalue_history.len() > MAX_HISTORY_SIZE {
            self.evalue_history.drain(0..MAX_HISTORY_SIZE / 2);
        }
