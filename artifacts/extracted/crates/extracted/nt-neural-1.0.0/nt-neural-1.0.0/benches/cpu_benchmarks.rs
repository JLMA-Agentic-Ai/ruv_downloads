...
fn generate_array1(size: usize) -> Array1<f64> {
    let mut rng = rand::thread_rng();
    Array1::from_vec((0..size).map(|_| rng.gen_range(-100.0..100.0)).collect())
}
...
fn benchmark_preprocessing(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000, 100000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("zscore", size), &data, |b, data| {
            b.iter(|| normalize_zscore(black_box(data)))
        });

        group.bench_with_input(BenchmarkId::new("minmax", size), &data, |b, data| {
            b.iter(|| normalize_minmax(black_box(data)))
        });

        group.bench_with_input(BenchmarkId::new("robust", size), &data, |b, data| {
            b.iter(|| normalize_robust(black_box(data)))
        });
...
fn fourier_features(data: &[f64], n_frequencies: usize) -> Vec<Vec<f64>> {
    ...
    for k in 1..=n_frequencies {
        ...
        let sin_feat: Vec<f64> = (0..n).map(|i| (freq * i as f64).sin()).collect();

        let cos_feat: Vec<f64> = (0..n).map(|i| (freq * i as f64).cos()).collect();
...
fn benchmark_feature_engineering(c: &mut Criterion) {
    ...
    for size in [1000, 5000, 10000, 50000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("ema", size), &data, |b, data| {
            b.iter(|| ema(black_box(data), 0.1))
        });

        group.bench_with_input(BenchmarkId::new("roc", size), &data, |b, data| {
            b.iter(|| rate_of_change(black_box(data), 14))
        });

        // Fourier features (only for smaller sizes)
        if *size <= 10000 {
            group.bench_with_input(BenchmarkId::new("fourier_5freq", size), &data, |b, data| {
                b.iter(|| fourier_features(black_box(data), 5))
            });
        }
    }
...
fn benchmark_training(c: &mut Criterion) {
    ...
    for (n_samples, n_features) in [(100, 10), (1000, 50), (10000, 100)].iter() {
        ...
        group.bench_function(&format!("epoch_s{}_f{}", n_samples, n_features), |b| {
            b.iter(|| {
                training_epoch(
                    black_box(&mut weights.clone()),
                    black_box(&inputs),
                    black_box(&targets),
                    0.01,
                )
    ...
    for (n_samples, n_features) in [(100, 10), (1000, 50), (10000, 100)].iter() {
        ...
        group.bench_function(&format!("gradients_s{}_f{}", n_samples, n_features), |b| {
            b.iter(|| {
                compute_gradients(black_box(&weights), black_box(&inputs), black_box(&targets))
            })
        });
    }
    ...
    for n_features in [10, 50, 100, 500].iter() {
        ...
        group.bench_function(&format!("param_update_f{}", n_features), |b| {
            b.iter(|| update_parameters(black_box(&mut weights), black_box(&gradients), 0.01))
        });
    }
...
fn allocation_benchmark(size: usize, count: usize) -> Vec<Vec<f64>> {
    (0..count).map(|_| vec![0.0; size]).collect()
}

fn clone_benchmark(data: &[f64], count: usize) -> Vec<Vec<f64>> {
    (0..count).map(|_| data.to_vec()).collect()
}
...
fn benchmark_memory(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        group.bench_function(&format!("allocation_size{}_count100", size), |b| {
            b.iter(|| allocation_benchmark(black_box(*size), 100))
        });
    }
    ...
    for size in [100, 1000, 10000, 100000].iter() {
        ...
        group.bench_function(&format!("clone_size{}_count10", size), |b| {
            b.iter(|| clone_benchmark(black_box(&data), 10))
        });
    }
    ...
    for size in [1000, 10000, 100000].iter() {
        ...
        group.bench_function(&format!("cache_efficient_sum_{}", size), |b| {
            b.iter(|| cache_efficient_sum(black_box(&data)))
        });
    }

    for dim in [100, 316, 1000].iter() {
        // 316^2 ≈ 100k
        let data = generate_array2(*dim, *dim);

        group.bench_function(&format!("cache_inefficient_sum_{}x{}", dim, dim), |b| {
            b.iter(|| cache_inefficient_sum(black_box(&data)))
        });
    }
