...
mod candle_tests {
    ...
    fn test_transformer_different_layers() {
        ...
        for (enc_layers, dec_layers) in [(1, 1), (2, 2), (3, 3), (4, 2)] {
            ...
            assert!(
                output.is_ok(),
                "Enc={}, Dec={} forward pass should succeed",
                enc_layers,
                dec_layers
            );
        }
    ...
    fn test_transformer_different_batch_sizes() {
        ...
        for batch_size in [1, 4, 8, 16, 32] {
            ...
            assert_eq!(
                output.dims()[0],
                batch_size,
                "Output batch size should match input for batch_size={}",
                batch_size
            );
        }
    }
    ...
    fn test_transformer_sequence_lengths() {
        ...
        for seq_len in [12, 24, 48, 96, 168] {
            ...
            assert!(
                output.is_ok(),
                "Forward with seq_len={} should succeed",
                seq_len
            );
        }
    }
    ...
    fn test_transformer_multivariate() {
        ...
        for num_features in [1, 3, 5, 10] {
            ...
            assert!(
                output.is_ok(),
                "{} features forward should succeed",
                num_features
            );
        }
    }
    ...
    fn test_transformer_deterministic() {
        ...
        for (v1, v2) in out1_vec.iter().zip(out2_vec.iter()) {
            assert!(
                (v1 - v2).abs() < 1e-5,
                "Model should be deterministic: {} vs {}",
                v1,
                v2
            );
        }
