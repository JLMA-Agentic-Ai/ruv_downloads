...
use nt_neural::storage::{
    AgentDbConfig, AgentDbStorage, ModelCheckpoint, ModelMetadata, SearchFilter,
};
...
async fn test_model_save_and_load() {
    ...
    assert_eq!(
        model_bytes, loaded_bytes,
        "Loaded model bytes should match original"
    );
...
async fn test_model_filtering() {
    ...
    assert_eq!(
        filtered_models.len(),
        1,
        "Should find exactly 1 NHITS model"
    );
...
async fn test_vector_similarity_search() {
    ...
    assert!(
        !results.is_empty(),
        "Should find at least one similar model"
    );
...
async fn test_model_with_metrics() {
    ...
    assert!(
        loaded_metadata.metrics.is_some(),
