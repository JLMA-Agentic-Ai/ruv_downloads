...
use nt_neural::{
    initialize, LSTMAttentionConfig, LSTMAttentionModel, ModelConfig, ModelType, NHITSConfig,
    NHITSModel, TransformerConfig, TransformerModel,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let input = Tensor::randn(0.0_f32, 1.0, (batch_size, input_size), &device)?;
    ...
    println!(
        "  NHITS             | {:>10} | {:?}",
        nhits_model.num_parameters(),
        nhits_output.dims()
    );
    println!(
        "  LSTM-Attention    | {:>10} | {:?}",
        lstm_model.num_parameters(),
        lstm_output.dims()
    );
    println!(
        "  Transformer       | {:>10} | {:?}",
        transformer_model.num_parameters(),
        transformer_output.dims()
    );
