...
use nt_neural::storage::{AgentDbConfig, AgentDbStorage, ModelCheckpoint, ModelMetadata};
...
async fn main() -> anyhow::Result<()> {
    ...
    let best_checkpoint = checkpoint_ids
        .iter()
        .min_by(|(_, _, loss_a), (_, _, loss_b)| loss_a.partial_cmp(loss_b).unwrap())
        .unwrap();
    ...
    let (loaded_checkpoint, loaded_state) = storage.load_checkpoint(&best_checkpoint.0).await?;
    ...
    if let Some(optimizer_state) = loaded_checkpoint.optimizer_state {
        println!(
            "     Optimizer: {}",
            serde_json::to_string_pretty(&optimizer_state)?
        );
    }
    println!();
    ...
    println!(
        "  Starting from epoch {} with val_loss {:.4}",
