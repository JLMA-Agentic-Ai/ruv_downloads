...
fn get_test_broker() -> AlpacaBroker {
    ...
    let api_key = env::var("ALPACA_API_KEY").expect("ALPACA_API_KEY not set");
    let secret_key = env::var("ALPACA_API_SECRET").expect("ALPACA_API_SECRET not set");
...
async fn test_get_account() {
    ...
    let account = broker.get_account().await.expect("Should get account info");
...
async fn test_get_positions() {
    ...
    let positions = broker.get_positions().await.expect("Should get positions");
...
async fn test_place_and_cancel_limit_order() {
    ...
    // Cancel it
    broker
        .cancel_order(&response.order_id)
        .await
...
async fn test_broker_creation() {
    let broker = AlpacaBroker::new("test_key".to_string(), "test_secret".to_string(), true);
