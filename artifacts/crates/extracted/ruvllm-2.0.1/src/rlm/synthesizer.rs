...
use super::decomposer::{SubQuery, DecompositionStrategy};
#[allow(unused_imports)]
use super::types::{SubAnswer, RlmAnswer, TokenUsage};
...
impl AnswerSynthesizer {
    ...
    pub fn synthesize(
        ...
        match self.strategy {
            ...
            AggregationStrategy::BestAnswer => {
                self.synthesize_best_answer(&sub_answers)
            }
            AggregationStrategy::Consensus => {
                self.synthesize_consensus(&sub_answers)
            }
        }
    }
    ...
    fn synthesize_concatenate(
        ...
        let avg_confidence = sub_answers.iter().map(|a| a.confidence).sum::<f32>()
            / num_answers as f32;
    ...
    fn synthesize_weighted_merge(
        ...
        sorted_answers.sort_unstable_by(|a, b| b.confidence.partial_cmp(&a.confidence).unwrap_or(std::cmp::Ordering::Equal));

        // Pre-calculate capacity
        let truncated_query = truncate_query_fast(original_query);
        let total_len: usize = sub_answers.iter().map(|a| a.answer.len() + 30).sum::<usize>() + 100;
        let mut text = String::with_capacity(total_len);
        ...
        match decomposition_strategy {
            ...
            DecompositionStrategy::Comparison(items) => {
                ...
                for (i, answer) in sorted_answers.iter().enumerate() {
                    if i < items.len() {
                        text.push_str("\n\n**");
                        text.push_str(items.get(i).map(|s| s.as_str()).unwrap_or(&answer.sub_query));
                        text.push_str("**\n");
                        text.push_str(&answer.answer);
                    }
                }
    ...
    fn synthesize_summarize(
        ...
        parts.push(format!("Summary for \"{}\":", truncate_query(original_query)));

        // Extract key points from each answer
        for answer in sub_answers {
            // Take first sentence as key point
            let first_sentence = answer.answer
                .split(&['.', '!', '?'][..])
                .next()
    ...
    fn synthesize_consensus(&self, sub_answers: &[SubAnswer]) -> Result<SynthesisResult> {
        ...
        let confident_answers: Vec<_> = sub_answers.iter()
        ...
        let avg_confidence = confident_answers.iter()
            .map(|a| a.confidence)
            .sum::<f32>() / confident_answers.len() as f32;
...
mod tests {
    ...
    fn test_synthesizer_single_answer() {
        ...
        let answers = vec![
            create_test_answer("What is AI?", "AI is artificial intelligence.", 0.9),
        ];
