...
impl RlmTrainingExample {
    ...
    pub fn from_trajectory(trajectory: &Trajectory) -> Self {
        ...
        for (i, step) in trajectory.steps.iter().enumerate() {
            let sub_query = SubQuery::new(i as u32, step.action.clone())
                .with_complexity(step.confidence);
...
impl RlmDataset {
    ...
    fn compute_stats(&mut self) {
        ...
        for example in &self.examples {
            ...
            *self.stats.strategy_distribution.entry(strategy).or_insert(0) += 1;
        }
...
mod tests {
    ...
    fn test_rlm_training_example() {
        ...
        example.add_sub_answer(SubAnswer::success(0, "Answer 1".to_string(), "coder").with_quality(0.8));
