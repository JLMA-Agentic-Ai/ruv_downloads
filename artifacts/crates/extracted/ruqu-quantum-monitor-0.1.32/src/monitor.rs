...
//!
//! ```ignore
//! use ruqu_quantum_monitor::{QuantumCoherenceMonitor, MonitorConfig};
//! use ndarray::Array2;
//!
//! // Create monitor
...
use crate::confidence::{ConfidenceInterval, ConfidenceSequence, ConfidenceSequenceConfig};
use crate::error::{QuantumMonitorError, Result};
use crate::evalue::{EValueConfig, EValueSummary, EValueTest};
...
impl QuantumCoherenceMonitor {
    ...
    pub fn observe(&mut self, sample: &Array1<f64>) -> Result<ObservationResult> {
        ...
        // Validate sample values are finite (no NaN or Inf)
        if sample.iter().any(|x| !x.is_finite()) {
            return Err(QuantumMonitorError::NumericalError(
                "Sample contains NaN or Inf values".to_string(),
            ));
        }

        // Update streaming kernel accumulator
        let accumulator = self.kernel_accumulator.as_mut().ok_or_else(|| {
            QuantumMonitorError::NotInitialized(
                "Kernel accumulator not initialized".to_string(),
            )
        })?;
