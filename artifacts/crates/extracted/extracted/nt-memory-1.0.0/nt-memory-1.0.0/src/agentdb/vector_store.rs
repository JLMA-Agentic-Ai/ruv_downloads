...
impl VectorStore {
    ...
    pub async fn ensure_collection(&self, name: &str, dimension: usize) -> anyhow::Result<()> {
    ...
    pub async fn search(
        ...
        let query = VectorQuery::new(collection.to_string(), query_embedding, top_k);
...
mod tests {
    ...
    async fn test_vector_store_operations() {
        let store = VectorStore::new("http://localhost:3000").await.unwrap();
        ...
        // Search
        let results = store.search("test_collection", embedding, 1).await.unwrap();
