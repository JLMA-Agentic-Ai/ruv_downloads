...
pub use agentdb::{EmbeddingProvider, VectorStore};
pub use cache::{CacheConfig, CacheEntry, HotCache};
pub use coordination::{ConsensusEngine, DistributedLock, Namespace, PubSubBroker};
pub use reasoningbank::{MemoryDistiller, Trajectory, TrajectoryTracker, Verdict, VerdictJudge};
...
impl MemorySystem {
    ...
    pub async fn new(config: MemoryConfig) -> Result<Self> {
        ...
        let cold_storage =
            sled::open(&config.storage_path).map_err(|e| MemoryError::Storage(e.to_string()))?;
