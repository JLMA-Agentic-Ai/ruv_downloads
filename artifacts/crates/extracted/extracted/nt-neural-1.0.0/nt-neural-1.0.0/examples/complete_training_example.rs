...
use nt_neural::{
    ...
    training::{
        DataLoader, OptimizerConfig, OptimizerType, TimeSeriesDataset, Trainer, TrainingConfig,
    },
...
async fn main() -> anyhow::Result<()> {
    ...
    let mut trainer =
        Trainer::new(training_config, device.clone()).with_checkpointing("checkpoints/nhits");
    ...
    println!(
        "   Final train loss: {:.6}",
        metrics.last().unwrap().train_loss
    );
    println!(
        "   Final val loss: {:.6}",
        metrics.last().unwrap().val_loss.unwrap()
    );
    ...
    println!(
        ...
        1000.0 / prediction.inference_time_ms
    );
    ...
    println!(
        "   Avg latency: {:.2}ms",
        batch_time.as_secs_f64() * 1000.0 / 100.0
    );
    ...
    println!(
        "  - Batch throughput: {:.0} samples/sec",
        100.0 / batch_time.as_secs_f64()
    );
