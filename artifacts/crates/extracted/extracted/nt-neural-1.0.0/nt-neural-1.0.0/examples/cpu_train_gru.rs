...
use nt_neural::training::cpu_trainer::{
    CPUTrainer, CPUTrainingConfig, OptimizerType, SimpleGRUWeights,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let metrics =
        trainer.train_gru(&mut weights, &train_x, &train_y, Some(&val_x), Some(&val_y))?;
