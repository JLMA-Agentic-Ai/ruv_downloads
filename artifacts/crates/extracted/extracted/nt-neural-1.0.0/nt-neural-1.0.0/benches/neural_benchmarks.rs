    ...
    for batch_size in [1, 16, 32].iter() {
        group.bench_with_input(
            ...
            |b, &batch_size| {
                ...
                // Create input tensor
                let input =
                    candle_core::Tensor::randn(0.0f32, 1.0, (batch_size, 168), &device).unwrap();
...
fn benchmark_model_comparison(c: &mut Criterion) {
    ...
    let transformer_model =
        TransformerModel::new(transformer_config, vb.pp("transformer")).unwrap();
...
fn benchmark_data_preprocessing(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000] {
        ...
        group.bench_with_input(BenchmarkId::new("denormalize", size), &size, |b, _| {
            b.iter(|| {
