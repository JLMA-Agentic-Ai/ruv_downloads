...
use nt_neural::utils::features::{ema, rolling_mean, rolling_std};
use nt_neural::utils::preprocessing::{min_max_normalize, normalize, NormalizationParams};
...
fn bench_min_max(c: &mut Criterion) {
    ...
    for size in [100, 1_000, 10_000, 100_000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("min_max_normalize", size),
            &data,
            |b, d| {
                b.iter(|| {
        ...
        #[cfg(feature = "simd")]
        {
            let params = NormalizationParams::from_data(&data);
            group.bench_with_input(
                BenchmarkId::new("simd_min_max_normalize", size),
                &data,
                |b, d| {
                    b.iter(|| {
                        let normalized =
                            simd_min_max_normalize(black_box(d), params.min, params.max);
                        black_box(normalized)
                    });
                },
            );
