...
fn test_detrending_with_noise() {
    ...
    let detrended_variance =
        detrended.iter().map(|x| x.powi(2)).sum::<f64>() / detrended.len() as f64;
...
fn test_rolling_std_calculation() {
    ...
    let mean1 = 2.0_f64;
    let expected_std1 =
        ((1.0_f64 - mean1).powi(2) + (2.0_f64 - mean1).powi(2) + (3.0_f64 - mean1).powi(2)) / 3.0;
...
fn test_seasonality_detection() {
    ...
    let data: Vec<f64> = (0..70).map(|i| 100.0 + 10.0 * (i % 7) as f64).collect();
...
mod helpers {
    ...
    pub fn generate_noisy_data(n: usize, noise_level: f64) -> Vec<f64> {
        ...
        (0..n)
            .map(|i| i as f64 + rng.gen::<f64>() * noise_level)
            .collect()
    }
}
