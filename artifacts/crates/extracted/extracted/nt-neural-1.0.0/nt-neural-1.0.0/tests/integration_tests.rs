...
mod candle_tests {
    ...
    async fn test_data_preprocessing_pipeline() {
        ...
        for (orig, denorm) in data.iter().zip(denormalized.iter()) {
            assert!(
                (orig - denorm).abs() < 1e-6,
                "Denormalization should restore original"
            );
        }
    ...
    async fn test_model_comparison() {
        ...
        let configs = vec![("small", 64), ("medium", 128), ("large", 256)];
    ...
    async fn test_inference_batch_processing() {
        ...
        let batch_predictor =
            nt_neural::inference::BatchPredictor::new(Box::new(model), 8).unwrap();
