...
mod simd_tests {
    ...
    use nt_neural::utils::preprocessing::{min_max_normalize, normalize};
    ...
    fn test_simd_multiply_accuracy() {
        ...
        for size in sizes {
            ...
            let scalar_multiply: Vec<f64> =
                data_a.iter().zip(&data_b).map(|(a, b)| a * b).collect();
            let simd_multiply_result = simd_multiply(&data_a, &data_b);

            for (i, (scalar, simd)) in scalar_multiply
                .iter()
                .zip(&simd_multiply_result)
                .enumerate()
            {
                let error = (scalar - simd).abs();
                assert!(
