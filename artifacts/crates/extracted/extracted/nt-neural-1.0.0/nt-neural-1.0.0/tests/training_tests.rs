...
mod candle_tests {
    ...
    async fn test_overfit_single_batch() {
        ...
        println!(
            "✅ Overfit test passed: Final loss = {:.6}",
            metrics.train_loss
        );
    }
    ...
    async fn test_training_convergence() {
        ...
        assert!(history.len() > 1, "Should have multiple epochs of metrics");
    ...
    async fn test_validation_metrics() {
        ...
        let test_dataset = TimeSeriesDataset::new(test_df, "close", 24, 12).unwrap();
    ...
    async fn test_gpu_vs_cpu_parity() {
        ...
        #[cfg(any(feature = "cuda", feature = "metal"))]
        {
            ...
            if let Ok(mut trainer_gpu) = NHITSTrainer::new(config_gpu) {
                let metrics_gpu = trainer_gpu.train_from_dataframe(df, "close").await.unwrap();

                println!("CPU loss: {:.6}", metrics_cpu.train_loss);
                println!("GPU loss: {:.6}", metrics_gpu.train_loss);

                // Results should be similar (within 10%)
                let diff_pct = (metrics_cpu.train_loss - metrics_gpu.train_loss).abs()
                    / metrics_cpu.train_loss
                    * 100.0;
