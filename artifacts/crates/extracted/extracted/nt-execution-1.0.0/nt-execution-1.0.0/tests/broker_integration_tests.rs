...
async fn test_ibkr_broker() {
    ...
    match broker.connect().await {
        ...
        Err(e) => {
            eprintln!(
                "❌ IBKR connection failed: {} (ensure TWS/Gateway is running)",
                e
            );
        }
    }
...
async fn test_polygon_client() {
    ...
    match client.get_last_quote("AAPL").await {
        Ok(quote) => {
            println!(
                "✅ Polygon quote for AAPL: ${:.2} (bid: ${:.2}, ask: ${:.2})",
...
async fn test_alpha_vantage() {
    ...
    match client.get_quote("AAPL").await {
        Ok(quote) => {
            println!(
                "✅ Alpha Vantage quote for {}: ${:.2} (change: {:.2}%)",
...
async fn test_news_api() {
    ...
    match client
        .search("Apple stock", None, None, Some("en"), Some("relevancy"))
        .await
    {
        Ok(articles) => {
            println!(
                "✅ NewsAPI found {} articles about Apple stock",
                articles.len()
            );
...
async fn test_yahoo_finance() {
    ...
    match client.get_quote("AAPL").await {
        Ok(quote) => {
            println!(
                "✅ Yahoo Finance quote for {}: ${:.2} (change: {:.2}%)",
                quote.symbol, quote.price, quote.change_percent
            );
        }
        Err(e) => {
