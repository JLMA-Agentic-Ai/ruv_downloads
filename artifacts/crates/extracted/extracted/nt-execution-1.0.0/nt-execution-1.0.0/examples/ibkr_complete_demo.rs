...
async fn main() -> anyhow::Result<()> {
    ...
    if positions.is_empty() {
        ...
        for pos in &positions {
            println!(
                "   {} | Qty: {} | Avg Entry: ${:.2} | Current: ${:.2} | P&L: ${:.2} ({:.2}%)",
                pos.symbol,
                pos.qty,
                pos.avg_entry_price,
                pos.current_price,
        ...
        Ok(chain) => {
            ...
            for contract in chain.iter().take(3) {
                ...
                if let Ok(greeks) = broker.get_option_greeks(contract).await {
                    println!(
                        "     Greeks: Δ={:.4} Γ={:.4} Θ={:.4} V={:.4} IV={:.2}%",
                        greeks.delta,
                        greeks.gamma,
                        greeks.theta,
                        greeks.vega,
                        greeks.implied_volatility * 100.0
                    );
                }
            }
        }
...
async fn trailing_stop_example(broker: &IBKRBroker) -> anyhow::Result<()> {
    // Percentage-based
    let response = broker
        .place_trailing_stop("AAPL", 100, OrderSide::Sell, TrailingStop::Percentage(5.0))
        .await?;
