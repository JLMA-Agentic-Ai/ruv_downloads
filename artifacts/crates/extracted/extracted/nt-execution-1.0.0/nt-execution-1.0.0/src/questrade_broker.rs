...
impl QuestradeBroker {
    ...
    pub async fn authenticate(&self) -> Result<(), BrokerError> {
        ...
        if response.status().is_success() {
            ...
            info!(
                "Questrade authentication successful, expires at {:?}",
                token.expires_at
            );
    ...
    async fn load_account_number(&self) -> Result<(), BrokerError> {
        ...
        let response: AccountsResponse = self
            .request_internal(Method::GET, "/v1/accounts", None::<()>)
            .await?;
        ...
        let primary_account = response
            ...
            .ok_or_else(|| {
                BrokerError::Other(anyhow::anyhow!("No active primary account found"))
            })?;
