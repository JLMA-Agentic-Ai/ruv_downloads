...
pub enum AlgoStrategy {
    ...
    PercentOfVolume { participation_rate: f64 },
}
...
impl IBKRBroker {
        ...
        if status.authenticated {
            ...
            *self.connection_status.write() = ConnectionStatus::Connected;
            ...
            Err(BrokerError::Auth(
                "Not authenticated with TWS/Gateway".to_string(),
            ))
        }
    }
    ...
    pub async fn get_option_chain(
        ...
        let contracts: Vec<OptionContract> = chain
            .strikes
            .into_iter()
            .flat_map(|strike| {
                vec![
        ...
        self.option_chains
            .insert(underlying.to_string(), contracts.clone());
        Ok(contracts)
    }
    ...
    pub async fn get_option_greeks(
        ...
        let cache_key = format!(
            "{}_{}_{:?}",
            contract.underlying, contract.expiry, contract.right
        );
        ...
        let greeks: IBKRGreeksResponse = self
            .request(
                Method::GET,
                &format!(
                    "/iserver/marketdata/snapshot?conids={}&fields=7283,7284,7285,7286,7287,7633",
                    conid
                ),
                None::<()>,
            )
    ...
    pub async fn place_option_order(
        ...
        let resp = response
            .first()
            .ok_or_else(|| BrokerError::Other(anyhow::anyhow!("No order response from IBKR")))?;
    ...
    pub async fn place_bracket_order(
        ...
        let conid = self
            .get_contract_id(&bracket.entry.symbol.to_string())
            .await?;
    ...
    async fn get_option_contract_id(&self, contract: &OptionContract) -> Result<i64, BrokerError> {
        ...
        let local_symbol = format!(
            "{}{}{}{}",
            contract.underlying, contract.expiry, right, contract.strike
        );
    ...
    async fn request<T: serde::de::DeserializeOwned>(
        ...
        match response.status() {
            ...
            StatusCode::SERVICE_UNAVAILABLE => Err(BrokerError::Unavailable(
                "IBKR service unavailable".to_string(),
            )),
            status => {
