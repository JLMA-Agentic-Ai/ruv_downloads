...
fn test_order_stop_limit() {
    let symbol = Symbol::new("AAPL").unwrap();
    let order = Order::stop_limit(symbol.clone(), Side::Sell, dec!(100), dec!(145), dec!(144));
...
fn test_order_book_best_bid_ask() {
    ...
    let book = OrderBook {
        symbol,
        timestamp: Utc::now(),
        bids: vec![(dec!(100.00), dec!(1000)), (dec!(99.95), dec!(2000))],
        asks: vec![(dec!(100.05), dec!(1500)), (dec!(100.10), dec!(2500))],
    };
