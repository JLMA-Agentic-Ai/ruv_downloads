...
export class SwarmCircuitExplorer {
    ...
    async explore() {
        ...
        for (let step = 0; step < this.config.explorationSteps; step++) {
            ...
            if (diversity < 0.01 && performanceHistory.length > 20) {
                const recentImprovement = Math.abs(performanceHistory[performanceHistory.length - 1] -
    ...
    generateRandomCircuit() {
        ...
        for (let layer = 0; layer < depth; layer++) {
            ...
            for (let qubit = 0; qubit < this.config.numQubits; qubit++) {
                const gateType = ['RX', 'RY', 'RZ'][Math.floor(Math.random() * 3)];
     ...
     */
    evaluateGenericCircuit(circuit) {
     ...
     */
    async querySimilarCircuits(circuit) {
        const embedding = this.circuitToEmbedding(circuit);
    ...
    async extractLearnedPatterns() {
        ...
        allCircuits.forEach((item) => {
            const gates = item.metadata.gates;
     ...
     */
    computeSwarmDiversity() {
        if (this.swarm.length < 2)
            return 1;
        let totalDistance = 0;
        let count = 0;
        for (let i = 0; i < this.swarm.length; i++) {
            for (let j = i + 1; j < this.swarm.length; j++) {
                const distance = this.circuitDistance(this.swarm[i].position, this.swarm[j].position);
     ...
     */
    circuitDistance(c1, c2) {
        const paramDist = Math.sqrt(c1.parameters.reduce((sum, p, i) => sum + Math.pow(p - c2.parameters[i], 2), 0));
 ...
 */
export async function exploreCircuits(config) {
    const fullConfig = {
...
//# sourceMappingURL=swarm-circuits.js.map