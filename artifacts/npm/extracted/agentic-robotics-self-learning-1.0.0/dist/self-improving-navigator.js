...
class SelfImprovingNavigator {
    ...
    loadMemoryBank() {
        ...
        if (existsSync(memoryPath)) {
            try {
                ...
                const navHistory = this.memoryBank.optimizationHistory?.filter((h) => h.type === 'navigation') || [];
    ...
    updateStrategiesFromHistory(history) {
    ...
    async executeNavigation(task, strategy) {
        ...
        while (steps < maxSteps) {
            ...
            const hasCollision = task.obstacles.some(obs => this.distance(nextPos, obs) < 0.5);
            if (hasCollision) {
                collisions++;
                // Avoid obstacle
                direction.x *= -1;
    ...
    calculateDirection(position, goal, obstacles, strategy) {
    ...
    evolveStrategies() {
        ...
        for (const key of Object.keys(newStrategy.parameters)) {
            ...
            newStrategy.parameters[key] *= (1 + mutation);
            newStrategy.parameters[key] = Math.max(0, Math.min(2, newStrategy.parameters[key]));
        }
...
//# sourceMappingURL=self-improving-navigator.js.map