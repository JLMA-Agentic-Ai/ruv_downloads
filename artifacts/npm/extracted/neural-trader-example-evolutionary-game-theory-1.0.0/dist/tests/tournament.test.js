...
import { ALWAYS_COOPERATE, ALWAYS_DEFECT, TIT_FOR_TAT, GRIM_TRIGGER, PAVLOV, } from '../src/strategies.js';
...
describe('Tournament', () => {
    ...
    describe('Match Playing', () => {
        ...
        it('should record scores correctly', () => {
            ...
            const defectPlayer = result.rankings.find((p) => p.id === ALWAYS_DEFECT.id);
            const cooperatePlayer = result.rankings.find((p) => p.id === ALWAYS_COOPERATE.id);
    ...
    describe('Match History', () => {
        it('should record match history', () => {
            ...
            tournament.run();
            const history = tournament.getMatchHistory(ALWAYS_COOPERATE.id, ALWAYS_DEFECT.id);
...
describe('quickTournament', () => {
    ...
    it('should use custom game', () => {
        const result = quickTournament([ALWAYS_COOPERATE, ALWAYS_DEFECT], PRISONERS_DILEMMA, 50);
        expect(result.rankings).toHaveLength(2);
    });
...
describe('Tournament Dynamics', () => {
    ...
    it('should show cooperation emergence', () => {
        ...
        const cooperativeCount = topTwo.filter((p) => [TIT_FOR_TAT.id, PAVLOV.id, ALWAYS_COOPERATE.id].includes(p.id)).length;
