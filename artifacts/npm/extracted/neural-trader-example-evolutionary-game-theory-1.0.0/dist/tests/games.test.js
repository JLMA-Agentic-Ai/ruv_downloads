 ...
 */
import { PRISONERS_DILEMMA, STAG_HUNT, PUBLIC_GOODS, ROCK_PAPER_SCISSORS, calculatePayoff, calculateFitnessValues, findPureNashEquilibria, createHawkDoveGame, createPublicGoodsGame, } from '../src/games.js';
describe('Game Definitions', () => {
    describe("Prisoner's Dilemma", () => {
        ...
        it('should have defection as dominant strategy', () => {
            ...
            expect(PRISONERS_DILEMMA.payoffMatrix[1][0]).toBeGreaterThan(PRISONERS_DILEMMA.payoffMatrix[0][0]); // D > C when opponent cooperates
            expect(PRISONERS_DILEMMA.payoffMatrix[1][1]).toBeGreaterThan(PRISONERS_DILEMMA.payoffMatrix[0][1]); // D > C when opponent defects
        });
    });
    ...
    describe('Stag Hunt', () => {
        ...
        it('should favor coordination on stag', () => {
            expect(STAG_HUNT.payoffMatrix[0][0]).toBeGreaterThan(STAG_HUNT.payoffMatrix[1][1]); // Stag-Stag > Hare-Hare
        });
    });
    describe('Public Goods Game', () => {
        it('should incentivize free-riding', () => {
            ...
            expect(game.payoffMatrix[1][0]).toBeGreaterThan(game.payoffMatrix[0][0]);
        });
        it('should have tragedy of commons', () => {
            const game = PUBLIC_GOODS;
            // Both contribute is better than both free-ride
            expect(game.payoffMatrix[0][0]).toBeGreaterThan(game.payoffMatrix[1][1]);
        });
    });
