...
export class Scheduler {
     ...
     */
    async generateSchedule(forecasts, constraints, startDate) {
        const shifts = [];
        const violations = [];
        ...
        for (let day = 0; day < daysToSchedule; day++) {
            for (let shiftNum = 0; shiftNum < shiftsPerDay; shiftNum++) {
                ...
                const shiftAssignments = this.assignStaffToShift(shiftStart, shiftEnd, shiftRequirements[day * shiftsPerDay + shiftNum], constraints, shifts);
     ...
     */
    calculateShiftRequirements(forecasts, constraints) {
        const requirements = [];
        const shiftsPerDay = 24 / this.config.shiftDuration;
     ...
     */
    assignStaffToShift(start, end, requirements, constraints, existingShifts) {
        const shifts = [];
        const violations = [];
        // Group staff by role
        ...
        for (const [role, requiredCount] of Object.entries(requirements)) {
            ...
            if (assignedCount < requiredCount) {
                violations.push(`Insufficient ${role} coverage: ${assignedCount}/${requiredCount} at ${start.toISOString()}`);
            }
        }
     ...
     */
    scoreStaffForShift(staff, start, end, existingShifts, constraints) {
        let score = 100;
     ...
     */
    calculateWeeklyHours(staffId, referenceDate, shifts) {
     ...
     */
    getLastShift(staffId, referenceDate, shifts) {
     ...
     */
    calculateConsecutiveHours(staffId, referenceDate, shifts) {
        const staffShifts = shifts
            .filter(s => s.staffId === staffId && s.end <= referenceDate)
            .sort((a, b) => b.end.getTime() - a.end.getTime());
     ...
     */
    calculateCoverageScore(shifts, requirements) {
        let totalRequired = 0;
        let totalMet = 0;
