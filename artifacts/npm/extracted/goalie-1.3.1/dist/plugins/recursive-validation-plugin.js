 ...
 */
export class RecursiveValidationPlugin {
     ...
     */
    recursiveValidate(state, depth = 0) {
        const reasons = [];
        let confidence = 1.0;
        ...
        // Check answer length
        const answer = state.final_answer || '';
        ...
        const forbiddenFound = this.validationCriteria.forbiddenTerms.filter(term => answer.toLowerCase().includes(term.toLowerCase()));
        ...
        if (this.validationCriteria.requiredDomains.length > 0) {
            ...
            const missingDomains = this.validationCriteria.requiredDomains.filter(domain => !citationDomains.some(cd => cd.includes(domain)));
    ...
    hooks = {
        ...
        onReplan: async (failedStep, state) => {
            ...
            const validationReasons = state.validation_reasons || [];
...
export function createValidationPlugin(criteria) {
    return new RecursiveValidationPlugin(criteria);
}
//# sourceMappingURL=recursive-validation-plugin.js.map