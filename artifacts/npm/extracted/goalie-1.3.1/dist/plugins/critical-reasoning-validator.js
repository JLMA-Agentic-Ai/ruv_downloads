 ...
 */
export class CriticalReasoningValidator {
    ...
    reasoningChecks = [
        {
            ...
            validator: async (content, citations) => {
                const issues = [];
            ...
            validator: async (content, citations) => {
                ...
                for (const pattern of claimPatterns) {
                    ...
                    for (const claim of matches) {
                        ...
                        const verified = citations.some(c => c.snippet && c.snippet.includes(claim.replace(/\$/g, '')));
                        if (!verified) {
                            unverifiedClaims++;
                        }
                    }
            ...
            validator: async (content, citations) => {
                ...
                if (citations.length > 0) {
                    const irrelevantCitations = citations.filter(c => {
                        ...
                        const relevance = this.calculateSimilarity(content.substring(0, 500), (c.title || '') + ' ' + (c.snippet || ''));
        ...
        },
        {
            ...
            validator: async (content, citations) => {
                ...
                const posCount = perspectiveMarkers.positive.filter(m => content.toLowerCase().includes(m)).length;
                const negCount = perspectiveMarkers.negative.filter(m => content.toLowerCase().includes(m)).length;
                const neutralCount = perspectiveMarkers.neutral.filter(m => content.toLowerCase().includes(m)).length;
                ...
                const addressedQuestions = questionWords.filter(q => content.toLowerCase().includes(q));
     ...
     */
    async performCriticalValidation(state, depth = 0) {
        ...
        if (depth < this.maxDepth - 1) {
            const researchSteps = state.research_steps || [];
            if (researchSteps.length > 0) {
                console.log(`\n   📊 Validating ${researchSteps.length} research steps...`);
                for (let i = 0; i < Math.min(researchSteps.length, 3); i++) {
                    const stepValidation = await this.performCriticalValidation({ ...state, final_answer: researchSteps[i].content || '' }, depth + 1);
    ...
    hooks = {
        ...
        afterSynthesize: async (result) => {
            ...
            const validation = await this.performCriticalValidation(result.state || result);
        ...
        onReplan: async (failedStep, state) => {
            ...
            const issues = state.critical_issues || [];
            if (issues.length > 0) {
...
export function createCriticalValidator() {
    return new CriticalReasoningValidator();
}
//# sourceMappingURL=critical-reasoning-validator.js.map