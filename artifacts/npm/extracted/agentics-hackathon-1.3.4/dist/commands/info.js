...
import { HACKATHON_NAME, HACKATHON_TAGLINE, HACKATHON_SPONSOR, HACKATHON_DESCRIPTION, TRACKS, DISCORD_URL, WEBSITE_URL, GITHUB_URL, BANNER } from '../constants.js';
...
export async function infoCommand(options = {}) {
    ...
    logger.newline();
    // Hackathon overview
    logger.box(`${chalk.bold(HACKATHON_NAME)}\n` +
        `${HACKATHON_TAGLINE}\n\n` +
    ...
    // What you'll build
    logger.box(`${chalk.bold('Technologies:')}\n` +
        `  • Google Gemini 2.5 Pro (1M token context)\n` +
        `  • Google Agent Development Kit (ADK)\n` +
        `  • Vertex AI & Google Cloud Platform\n` +
        `  • Claude Code & Claude Flow\n` +
    ...
    // Quick start
    logger.box(`${chalk.bold('1.')} Initialize your project:\n` +
        ...
        `${chalk.bold('4.')} Start building!`, 'Quick Start');
    logger.newline();
}
