...
const prompt = (options) => enquirerPrompt(options);
import chalk from 'chalk';
import ora from 'ora';
import path from 'path';
import { HACKATHON_NAME, TRACKS, AVAILABLE_TOOLS, DISCORD_URL, WEBSITE_URL, BANNER, WELCOME_MESSAGE } from '../constants.js';
import { logger, configExists, loadConfig, saveConfig, checkPrerequisites, installTool } from '../utils/index.js';
/**
...
async function runInteractive(options) {
    // Project name
    const { projectName: rawProjectName } = await prompt({
        type: 'input',
        name: 'projectName',
    ...
    // Discord
    const { joinDiscord } = await prompt({
        type: 'confirm',
        name: 'joinDiscord',
    ...
    if (joinDiscord) {
        logger.newline();
        logger.box(`Join our Discord community for:\n` +
            `  • Team formation\n` +
            `  • Technical support\n` +
            `  • Announcements\n` +
            `  • Networking\n\n` +
            chalk.cyan.underline(DISCORD_URL), 'Discord Community');
    }
    return {
        projectName,
...
function showSummary(config) {
    logger.newline();
    logger.divider();
    logger.box(`${chalk.bold('Project:')} ${config.projectName}\n` +
        ...
        Object.entries(config.tools)
            ...
            .join('\n') || '  • None', `${HACKATHON_NAME} - Setup Complete`);
    logger.newline();
