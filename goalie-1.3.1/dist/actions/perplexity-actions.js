...
export class PerplexityClient {
    ...
    async search(params) {
        ...
        if (params.recency)
            searchBody.search_recency_filter = params.recency;
        if (params.domains)
            searchBody.search_domain_filter = params.domains;
        if (params.maxResults)
            searchBody.max_results = params.maxResults;
 ...
 */
export const composeQueriesAction = {
    ...
    async execute(state, params) {
        try {
            ...
            queries.push(`${userQuery} research`, `${userQuery} latest developments`);
 ...
 */
export const searchInformationAction = {
 ...
 */
export const synthesizeResultsAction = {
    ...
    async execute(state, params) {
        try {
            ...
            const context = searchResults.map((result, index) => `[${index + 1}] ${result.title}\n${result.snippet}\nURL: ${result.url}\n`).join('\n');
            const messages = [
                {
                    role: 'system',
 ...
 */
function extractCitations(answer, searchResults) {
    const citations = [];
...
export const verifyAnswerAction = {
    ...
    async execute(state, params) {
        try {
            ...
            const uniqueDomains = new Set(citations.map(c => new URL(c.url).hostname));
...
//# sourceMappingURL=perplexity-actions.js.map