...
export class GoapMCPTools {
     ...
     * Execute GOAP search
     */
    async executeGoapSearch(params) {
        ...
        try {
            ...
            const result = await this.planner.executePlan(plan, this.availableActions, (newPlan) => {
            ...
            let answer = result.finalState.final_answer || 'No answer generated';
            if (answer.length > maxAnswerLength) {
                answer = answer.substring(0, maxAnswerLength) + '\n\n[Answer truncated for size. Full answer available in saved files.]';
            }
            const citations = result.finalState.citations || [];
            const usage = result.finalState.usage || { tokens: 0, cost: 0 };
            const verificationNotes = result.finalState.verification_notes || [];
            ...
            if (params.ed25519Verification?.enabled) {
                ...
                if (citations.length > 0) {
                    const citationVerification = await this.antiHallucinationVerifier.verifyCitations(citations);
                ...
                if (signResult && privateKey && keyId) {
                    const signedContent = await this.antiHallucinationVerifier.signSearchResult({ answer, citations, metadata: { planId: plan.id } });
            ...
            if (params.outputToFile) {
                const savedFiles = await this.outputManager.saveToFile(fullResult, params.query, params.outputFormat || 'markdown', {
            ...
            await this.pluginRegistry.executeOnError(error instanceof Error ? error : new Error('Unknown error'), { params, startTime });
            throw error;
        }
    }
    /**
     ...
     */
    async executePlanExplain(params) {
        const initialState = {
            user_query: params.query,
            queries_composed: false,
            information_searched: false,
     ...
     */
    generatePlanLog(result, reasoning) {
        const log = [];
