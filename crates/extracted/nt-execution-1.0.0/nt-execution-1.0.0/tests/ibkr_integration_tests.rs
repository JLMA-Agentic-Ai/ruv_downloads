...
async fn test_trailing_stop_percentage() {
    ...
    let response = broker
        .place_trailing_stop("AAPL", 1, OrderSide::Sell, TrailingStop::Percentage(5.0))
        .await;
...
async fn test_historical_data() {
    ...
    let bars = broker.get_historical_data("AAPL", "1d", "1min").await;
