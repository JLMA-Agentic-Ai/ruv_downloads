...
impl FillReconciler {
    ...
    pub async fn reconcile(
        ...
        if let Some(filled_price) = actual.filled_avg_price {
            if let Some((min_price, max_price)) = expected.expected_price_range {
                if filled_price < min_price || filled_price > max_price {
                    ...
                    let deviation =
                        ((filled_price - mid_price).abs() / mid_price) * Decimal::from(100);
        ...
        if let Some(filled_at) = actual.filled_at {
            let execution_time = (filled_at - expected.placed_at).num_milliseconds().abs();

            if execution_time > 60000 {
                // > 1 minute
                discrepancies.push(Discrepancy::TimingAnomaly {
                    message: format!("Order took {} seconds to fill", execution_time / 1000),
                });
            }
        }
        ...
        match result.status {
            ...
            ReconciliationStatus::Warning => {
                warn!(
                    "Order {} reconciliation warnings: {:?}",
                    order_id, result.discrepancies
                );
            }
            ReconciliationStatus::Error => {
                error!(
