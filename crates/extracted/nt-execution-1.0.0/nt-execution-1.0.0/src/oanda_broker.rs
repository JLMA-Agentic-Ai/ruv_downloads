...
impl BrokerClient for OANDABroker {
    async fn get_account(&self) -> Result<Account, BrokerError> {
        ...
        let balance = Decimal::from_str_exact(&response.account.balance).unwrap_or_default();
        let nav = Decimal::from_str_exact(&response.account.nav).unwrap_or_default();
        let margin_available =
            Decimal::from_str_exact(&response.account.margin_available).unwrap_or_default();
    ...
    async fn get_positions(&self) -> Result<Vec<Position>, BrokerError> {
        ...
        for pos in response.positions {
            ...
            if long_units != 0 {
                let avg_price =
                    Decimal::from_str_exact(&pos.long.average_price).unwrap_or_default();
                let unrealized_pl =
                    Decimal::from_str_exact(&pos.long.unrealized_pl).unwrap_or_default();

                positions.push(Position {
                    symbol: Symbol::new(pos.instrument.as_str())
                        .expect("Invalid symbol from OANDA"),
                    qty: long_units,
                    ...
                    unrealized_plpc: if avg_price != Decimal::ZERO {
                        (unrealized_pl / (avg_price * Decimal::from(long_units)))
                            * Decimal::from(100)
                    } else {
                        Decimal::ZERO
                    },
            ...
            if short_units != 0 {
                let avg_price =
                    Decimal::from_str_exact(&pos.short.average_price).unwrap_or_default();
                let unrealized_pl =
                    Decimal::from_str_exact(&pos.short.unrealized_pl).unwrap_or_default();
                ...
                positions.push(Position {
                    ...
                    unrealized_pl,
                    unrealized_plpc: if avg_price != Decimal::ZERO {
                        (unrealized_pl / (avg_price * Decimal::from(short_units.abs())))
                            * Decimal::from(100)
                    } else {
                        Decimal::ZERO
                    },
        ...
        let _: serde_json::Value = self
            .request(
                ...
                &format!(
                    ...
                    self.config.account_id, order_id
                ),
                None::<()>,
            )
            .await?;
