...
fn benchmark_preprocessing(c: &mut Criterion) {
    ...
    group.bench_function("normalization", |b| {
        b.iter(|| {
            ...
            let variance = test_input.iter().map(|x| (x - mean).powi(2)).sum::<f64>()
        ...
        b.iter(|| {
            ...
            for &lag in &lags {
                let lagged: Vec<f64> = (0..test_input.len())
                    .map(|i| if i >= lag { test_input[i - lag] } else { 0.0 })
                    .collect();
...
fn benchmark_cache_effects(c: &mut Criterion) {
    ...
    group.bench_function("warm_cache", |b| {
        b.iter(|| {
            ...
            {
                let tensor =
                    candle_core::Tensor::from_vec(test_input.clone(), (1, input_size, 1), &device)
                        .unwrap();
                let output = model.forward(&tensor).unwrap();
                black_box(output);
            }
