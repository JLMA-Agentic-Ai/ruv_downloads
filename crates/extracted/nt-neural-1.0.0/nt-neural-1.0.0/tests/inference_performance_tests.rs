...
fn test_gru_single_prediction_latency() {
    ...
    let latencies: Vec<u128> = (0..10)
        .map(|_| {
            measure_single_prediction_latency(&model, &test_input, &device, (1, INPUT_SIZE, 1))
        })
        .collect();
...
fn test_gru_batch_throughput() {
    ...
    let throughput = measure_batch_throughput(&model, 32, INPUT_SIZE, &device, |size| (1, size, 1));
