...
use anyhow::Result;
use nt_neural::{
    utils::{features::*, metrics::EvaluationMetrics, preprocessing::*},
...
async fn main() -> Result<()> {
    ...
    let (train_data, val_data) = (&features[..split_idx], &features[split_idx..]);
    ...
    let trained_model = trainer.train(&model, train_data, Some(val_data)).await?;
...
fn save_norm_params(
    params: &nt_neural::utils::preprocessing::NormParams,
    path: &str,
) -> Result<()> {
