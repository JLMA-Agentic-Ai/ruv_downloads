...
use nt_neural::storage::{
    AgentDbConfig, AgentDbStorage, ModelMetadata, SearchFilter, SimilarityMetric,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let similar_models = storage.search_similar_models(&query_embedding, 3).await?;
    ...
    for (i, result) in similar_models.iter().enumerate() {
        ...
        println!(
            "       Type: {}, Tags: {:?}",
            result.metadata.model_type, result.metadata.tags
        );
    }
    println!();
    ...
    println!(
        "  Found {} models with 'crypto' or 'hourly' tags:",
        tagged_models.len()
    );
