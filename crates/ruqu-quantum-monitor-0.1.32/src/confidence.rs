...
impl ConfidenceSequence {
    ...
    pub fn update(&mut self, observation: f64) -> Option<ConfidenceInterval> {
        ...
        // Bound history size to prevent memory exhaustion
        const MAX_HISTORY_SIZE: usize = 100_000;
        if self.mean_history.len() > MAX_HISTORY_SIZE {
            self.mean_history.drain(0..MAX_HISTORY_SIZE / 2);
        }

        if self.n < self.config.min_samples {
            self.width_history.push(f64::INFINITY);
            if self.width_history.len() > MAX_HISTORY_SIZE {
                self.width_history.drain(0..MAX_HISTORY_SIZE / 2);
            }
            return None;
        }
