...
use ruvllm::rlm::{
    RlmConfig, RlmController, NativeEnvironment, MemoryConfig,
    QueryResult, MemoryEntry,
};
...
fn test_controller_creation_with_default_config() {
    ...
    assert_eq!(stats.total_queries.load(std::sync::atomic::Ordering::Relaxed), 0);
    assert_eq!(stats.total_memories.load(std::sync::atomic::Ordering::Relaxed), 0);
}
...
fn test_stats_cloning() {
    ...
    stats.total_queries.fetch_add(10, std::sync::atomic::Ordering::SeqCst);

    // Clone should preserve value
    let cloned = (*stats).clone();
    assert_eq!(cloned.total_queries.load(std::sync::atomic::Ordering::SeqCst), 10);
}
...
fn test_controller_thread_safe() {
    ...
    let controller = Arc::new(
        RlmController::<NativeEnvironment>::new(RlmConfig::default()).unwrap()
    );
    ...
    for _ in 0..4 {
        ...
        let handle = thread::spawn(move || {
            ...
            stats.total_queries.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            stats.snapshot()
        });
        handles.push(handle);
    }
