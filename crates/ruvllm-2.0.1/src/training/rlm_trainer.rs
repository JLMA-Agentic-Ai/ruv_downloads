...
impl RlmTrainer {
    ...
    fn train_decomposition_batch(&mut self, batch: &TrainingBatch) -> Result<GrpoUpdateResult> {
        ...
        // Create sample group
        let group = SampleGroup::new(samples.clone(), self.state.current_step as u64, "decomposition".to_string());
        ...
        self.optimizer.grpo_update(&log_probs, &advantages, &ref_log_probs)
    }
    ...
    fn example_to_decomposition_sample(&self, example: &RlmTrainingExample) -> GrpoSample {
        ...
        let complexity_match = 1.0 - (example.decomposition.total_complexity - 1.0).abs().min(1.0);

        let reward = decomp_quality * 0.6 + depth_factor * 0.2 + complexity_match * 0.2;

        GrpoSample {
            state: example.query_embedding.clone().unwrap_or_else(|| vec![0.0; 768]),
    ...
    fn train_synthesis_batch(&mut self, batch: &TrainingBatch) -> Result<GrpoUpdateResult> {
        ...
        let group = SampleGroup::new(samples.clone(), self.state.current_step as u64, "synthesis".to_string());
        self.optimizer.add_group(group);
    ...
    fn example_to_synthesis_sample(&self, example: &RlmTrainingExample) -> GrpoSample {
        ...
        GrpoSample {
            state: example.final_embedding.clone().unwrap_or_else(|| vec![0.0; 768]),
    ...
    fn record_routing_trajectory(&self, pair: &ContrastivePair, loss: f32) -> Result<()> {
        if let Some(sona) = &self.sona {
            ...
            let trajectory = Trajectory {
                ...
                query_embedding: pair.anchor_embedding.clone().unwrap_or_else(|| vec![0.0; 768]),
