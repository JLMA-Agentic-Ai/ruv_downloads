...
impl QueryDecomposer {
    ...
    fn compute_complexity_fast(&self, query: &str, query_len: usize) -> f32 {
        ...
        if query_len > 10 {
            ...
            let conjunction_count = self.config.conjunction_keywords.iter()
            ...
            let clause_count = CLAUSE_INDICATORS.iter()
    ...
    #[inline]
    fn try_comparison_decomposition_fast(&self, query: &str, query_lower: &str) -> Option<DecompositionResult> {
        ...
        for keyword in &self.config.comparison_keywords {
            if let Some(pos) = query_lower.find(keyword.as_str()) {
                ...
                if parts.len() >= 2 {
                    DECOMPOSER_STATS.record_decomposition(DecompositionStrategy::Comparison(parts.clone()));
    ...
    #[inline]
    fn try_conjunction_decomposition_fast(&self, query: &str, query_lower: &str) -> Option<DecompositionResult> {
        for keyword in &self.config.conjunction_keywords {
            if let Some(_) = query_lower.find(keyword.as_str()) {
                ...
                if parts.len() >= 2 {
                    DECOMPOSER_STATS.record_decomposition(DecompositionStrategy::Conjunction(parts.clone()));
    ...
    #[inline]
    fn try_sequential_decomposition_fast(&self, query: &str, query_lower: &str) -> Option<DecompositionResult> {
        for keyword in &self.config.sequential_keywords {
            if let Some(_) = query_lower.find(keyword.as_str()) {
                let parts = self.split_by_keyword_fast(query, query_lower, keyword);
                if parts.len() >= 2 {
                    DECOMPOSER_STATS.record_decomposition(DecompositionStrategy::Sequential(parts.clone()));
    ...
    fn try_comparison_decomposition(&self, query: &str, query_lower: &str) -> Option<DecompositionResult> {
        self.try_comparison_decomposition_fast(query, query_lower)
    }

    fn try_conjunction_decomposition(&self, query: &str, query_lower: &str) -> Option<DecompositionResult> {
        self.try_conjunction_decomposition_fast(query, query_lower)
    }

    fn try_sequential_decomposition(&self, query: &str, query_lower: &str) -> Option<DecompositionResult> {
        self.try_sequential_decomposition_fast(query, query_lower)
    }

    /// Extract comparison entities at a known position.
    #[inline]
    fn extract_comparison_entities_at(&self, query: &str, keyword: &str, pos: usize) -> Vec<String> {
    ...
    pub fn record_decomposition(&self, strategy: DecompositionStrategy) {
        ...
        match &strategy {
            ...
            DecompositionStrategy::Conjunction(parts) => {
                ...
                self.total_sub_queries.fetch_add(parts.len() as u64, Ordering::SeqCst);
            }
...
mod tests {
    ...
    fn test_conjunction_decomposition() {
        ...
        assert!(matches!(result.strategy, DecompositionStrategy::Conjunction(_)));
    ...
    fn test_comparison_decomposition() {
        ...
        assert!(matches!(result.strategy, DecompositionStrategy::Comparison(_)));
    }
    ...
    fn test_query_classification() {
        ...
        assert_eq!(decomposer.classify_query("What is machine learning?"), QueryType::Definition);
        assert_eq!(decomposer.classify_query("Why does gravity exist?"), QueryType::Causal);
        assert_eq!(decomposer.classify_query("How do I learn programming?"), QueryType::Procedural);
        assert_eq!(decomposer.classify_query("Compare Python and JavaScript"), QueryType::Comparison);
    }
    ...
    fn test_decomposition_result_methods() {
        ...
        let complex_result = decomposer.decompose("What are the causes and effects of climate change?");
