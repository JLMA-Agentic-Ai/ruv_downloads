...
use super::controller::{RlmConfig, RlmController, QueryResult, MemoryEntry, MemoryMetadata, TokenUsage, SourceAttribution};
...
impl From<QueryResult> for JsQueryResult {
    fn from(r: QueryResult) -> Self {
        Self {
            ...
            sources: r.sources.into_iter().map(|s| JsSourceAttribution {
...
impl WasmRlmController {
    ...
    pub fn with_config(config: JsValue) -> Result<WasmRlmController, JsValue> {
        ...
        let js_config: JsRlmConfig = serde_wasm_bindgen::from_value(config)
            .unwrap_or_default();
    ...
    pub fn query(&self, input: &str) -> Result<JsValue, JsValue> {
        ...
        let result = controller.query(input)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    pub fn add_memory(&self, text: &str, metadata: JsValue) -> Result<String, JsValue> {
        ...
        controller.add_memory(text, metadata)
            .map_err(|e| JsValue::from_str(&e.to_string()))
    }
    ...
    pub fn search_memory(&self, query: &str, top_k: usize) -> Result<JsValue, JsValue> {
        let controller = self.inner.read();
        let results = controller.search_memory(query, top_k)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let js_results: Vec<JsSearchResult> = results.into_iter()
            .map(|r| r.into())
            .collect();
    ...
    pub fn get_memory(&self, id: &str) -> Result<JsValue, JsValue> {
        let controller = self.inner.read();
        let entry = controller.get_memory(id)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
    ...
    pub fn list_memories(&self, limit: usize, offset: usize) -> Result<JsValue, JsValue> {
        let controller = self.inner.read();
        let entries = controller.list_memories(limit, offset)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;

        let js_entries: Vec<JsSearchResult> = entries.into_iter()
    ...
    pub fn get_config(&self) -> Result<JsValue, JsValue> {
        ...
        serde_wasm_bindgen::to_value(&js_config)
            .map_err(|e| JsValue::from_str(&e.to_string()))
    }
}
