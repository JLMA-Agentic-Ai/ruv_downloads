...
pub use config::{
    AggregationStrategy, ConfigValidationError, DecompositionConfig,
    RecursiveConfig, RecursiveConfigBuilder,
};
...
pub use controller::{
    MemoryEntry, MemoryMetadata, QueryResult, RlmController, RlmStats, RlmStatsSnapshot,
    SourceAttribution, TokenUsage as ControllerTokenUsage, RlmConfig,
};
...
pub use traits::{
    FinishReason as TraitsFinishReason, GenerationOutput as TraitsGenerationOutput,
    GenerationParams as TraitsGenerationParams, LlmBackendAsync, LlmBackendTrait,
    MemoryMetadata as TraitsMemoryMetadata, MemorySpan as TraitsMemorySpan, MemoryStore,
    QueryContext as TraitsQueryContext, QueryDecomposition as TraitsQueryDecomposition,
    RlmAnswer as TraitsRlmAnswer, RlmConfig as TraitsRlmConfig, RlmModelInfo,
    SubAnswer as TraitsSubAnswer, SubQuery as TraitsSubQuery,
    DecompositionStrategy as TraitsDecompositionStrategy, TokenUsage as TraitsTokenUsage,
    StreamToken,
};
...
pub use simd_ops::{
    ...
    batch_cosine_similarity, batch_cosine_similarity_slices, cosine_similarity,
    cosine_similarity_prenorm, dot_product, l2_norm, normalize, normalize_inplace,
    // Batch SIMD operations (4-8 vectors in parallel)
    batch_cosine_similarity_4, batch_cosine_similarity_8, batch_cosine_similarity_4_prenorm,
    batch_dot_products, batch_dot_products_4, batch_l2_norms, batch_l2_norms_4,
    batch_normalize_inplace, batch_similarity_search,
};
...
pub use shared_types::{
    // Core shared types
    SharedText, SharedEmbedding, SharedBytes,
    // Conversion traits
    IntoSharedText, IntoSharedEmbedding,
    // Zero-copy query result
    SharedQueryResult, SharedSourceAttribution, SharedTokenUsage,
    // Zero-copy memory types
    SharedMemoryEntry, SharedMemoryMetadata, SharedSearchResult,
    // Cache types
    ArcCachedResponse,
    // Utility functions
    normalize_query, prepare_cache_key, extract_excerpt,
};
...
pub use pool::{
    ...
    VectorPool, PooledVec,
    // String pool for query processing
    StringPool, PooledString,
    // Generic result pool
    ResultPool, PooledResults,
    // Pool management
    PoolManager, PoolStats, PoolStatsSnapshot, CombinedPoolStats,
};
...
pub use ruvltra_backend::{
    ...
    RuvLtraRlmBackend, RuvLtraRlmConfig, RuvLtraEnvironment, RuvLtraEnvConfig,
    RuvLtraMemoryStore, RuvLtraBackendStats,
    // KV cache
    KvCache, KvCacheEntry, KvCacheStats,
    // Embedding pooling
    EmbeddingPooling,
    // Query decomposer and synthesizer from ruvltra_backend
    QueryDecomposer as RuvLtraQueryDecomposer, AnswerSynthesizer as RuvLtraSynthesizer,
};
