...
impl MockQueryDecomposer {
    ...
    fn compute_complexity(&self, query: &str) -> f32 {
        ...
        // Conjunction factor
        let conj_count = self.conjunction_keywords.iter()
        ...
        let q_count = question_words.iter()
    ...
    fn split_comparison(&self, query: &str, _keyword: &str) -> Vec<String> {
        vec![
            format!("What is {}?", query.split_whitespace().take(3).collect::<Vec<_>>().join(" ")),
            format!("What is {}?", query.split_whitespace().rev().take(3).collect::<Vec<_>>().join(" ")),
        ]
    }
...
impl MockCache {
    ...
    fn insert(&mut self, query: &str, answer: String, quality: f32) {
        ...
        self.entries.insert(query.to_lowercase(), CacheEntry {
            answer,
            quality,
...
impl MockQualityScorer {
    ...
    fn score_completeness(&self, answer: &str) -> f32 {
        let len = answer.len();
        if len < 10 { 0.2 }
        else if len > 2000 { 0.8 }
        else { 1.0 }
    }

    fn score_coherence(&self, answer: &str) -> f32 {
        let sentences: Vec<&str> = answer.split(|c| c == '.' || c == '!')
            .filter(|s| !s.trim().is_empty())
            .collect();
        if sentences.len() >= 2 { 0.9 } else { 0.7 }
    }
    ...
    fn score_relevance(&self, query: &str, answer: &str) -> f32 {
        ...
        if query_words.is_empty() { 0.5 }
        else { 0.3 + 0.5 * (matched as f32 / query_words.len() as f32) }
    }
...
fn bench_embedding_generation(c: &mut Criterion) {
    ...
    for dim in [256, 384, 768, 1024] {
        ...
        group.throughput(Throughput::Elements(1));
        group.bench_with_input(id, &embedder, |b, e| {
...
fn bench_batch_decomposition(c: &mut Criterion) {
    ...
    for batch_size in [10, 50, 100] {
        ...
        group.bench_with_input(id, &batch, |b, queries| {
            b.iter(|| {
                queries.iter().map(|q| decomposer.decompose(q)).collect::<Vec<_>>()
            })
        });
    }
...
fn bench_end_to_end_cached(c: &mut Criterion) {
    ...
    for query in &queries {
        cache.insert(
            query,
            format!("Cached answer for: {}", query),
            0.9,
        );
    }
...
impl MockVectorPool {
    ...
    fn acquire(&self) -> Vec<f32> {
        ...
        guard.pop().unwrap_or_else(|| Vec::with_capacity(self.dimension))
    }
...
impl ArcCache {
    ...
    fn insert(&mut self, key: u64, result: MockQueryResult) {
        self.entries.insert(key, MockArcCacheEntry {
    ...
    fn get(&self, key: &u64) -> Option<std::sync::Arc<MockQueryResult>> {
        self.entries.get(key).map(|e| std::sync::Arc::clone(&e.result)) // Arc clone only
    }
}
    ...
    // Measure the cost of Arc::try_unwrap path
    {
        ...
        group.bench_function("cache_hit_arc_unwrap", |b| {
            b.iter(|| {
                ...
                let result = Arc::try_unwrap(cached_arc)
