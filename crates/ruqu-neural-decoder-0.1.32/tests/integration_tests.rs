...
fn test_decoder_consistency() {
    ...
    let mut decoder1 = NeuralDecoder::new(config.clone()).unwrap();
    let mut decoder2 = NeuralDecoder::new(config).unwrap();
...
fn test_gnn_with_surface_code_graph() {
    ...
    let encoder = GNNEncoder::new(gnn_config).unwrap();
...
fn test_empty_graph_handling() {
    ...
    let graph = ruqu_neural_decoder::graph::DetectorGraph::new(3);
